...
static func panel(title: String = "", close_text: String = "", on_close: Callable = Callable(),
	...
	var cols := 2 if max_width >= Dp.px(940) else 1
	var per := int(ceil(LINE_KEYS.size() / float(cols)))
	...
	for c in cols:
		...
		for i in range(c * per, mini((c + 1) * per, LINE_KEYS.size())):
			var e: Array = LINE_KEYS[i]
			var row := HBoxContainer.new()
			row.add_theme_constant_override("separation", int(Dp.px(8)))
			var a := Label.new()
			a.text = TranslationServer.translate(e[0])
			a.custom_minimum_size = Vector2(Dp.px(160), 0)
			a.add_theme_font_size_override("font_size", int(Dp.px(12)))
			a.add_theme_color_override("font_color", HudTheme.GOLD)
			var b := Label.new()
			b.text = TranslationServer.translate(e[1])
			b.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
			b.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			b.custom_minimum_size = Vector2(Dp.px(250), 0)
	...
	list.custom_minimum_size = Vector2(Dp.px(420 * cols + 20 * (cols - 1)), room)

...
func _load_palette(path: String, player_colors: Array[Color]) -> void:
	...
	terrain_row = dim_offset * 2
	palette_rows = terrain_row + 1
...
static func _apply_remap(img: Image, row: int, base: Color) -> void:
	for i in REMAP_COUNT:
		var t := float(i) / float(REMAP_COUNT - 1)
		var v := lerpf(1.0, 0.25, t)
		var c := Color.from_hsv(base.h, base.s, base.v * v, 1.0)
		img.set_pixel(REMAP_FIRST + i, row, c)

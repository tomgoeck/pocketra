...
var atlas_textures: Array[ImageTexture] = []

var _page_raws: Array[PackedByteArray] = []
...
const TABLE_WIDTH := 128


static func _force_web_pages() -> bool:
	return OS.get_cmdline_user_args().has("--web-atlas")


func _load_atlas(base: String) -> void:


	var web_base := base + "_web"
	var use_web_pages := (OS.has_feature("web") or _force_web_pages()) \
			and FileAccess.file_exists(web_base + ".json")
	meta = JSON.parse_string(FileAccess.get_file_as_string((web_base if use_web_pages else base) + ".json"))
	var width: int = meta["width"]
	var height: int = meta["height"]
	if use_web_pages:
		for page in int(meta.get("pages", 0)):
			var page_raw := FileAccess.get_file_as_bytes(web_base + "_%d.r8" % page)
			assert(page_raw.size() == width * height, "Web-Atlasseite passt nicht zu den Metadaten")
			_page_raws.append(page_raw)
			atlas_textures.append(ImageTexture.create_from_image(
					Image.create_from_data(width, height, false, Image.FORMAT_R8, page_raw)))
		if atlas_textures.is_empty():


			push_error("Web-Atlas %s nennt keine Seiten — zurück auf das einseitige Blatt" % web_base)
			use_web_pages = false
			meta = JSON.parse_string(FileAccess.get_file_as_string(base + ".json"))
			width = meta["width"]
			height = meta["height"]
	if not use_web_pages:
		var raw := FileAccess.get_file_as_bytes(base + ".r8")
		assert(raw.size() == width * height, "Atlasgröße passt nicht zu den Metadaten")
		_page_raws.append(raw)
		atlas_textures.append(ImageTexture.create_from_image(
				Image.create_from_data(width, height, false, Image.FORMAT_R8, raw)))
	if atlas_textures.size() > MAX_PAGES:

		push_error("Atlas %s hat %d Seiten, der Shader kann %d" % [base, atlas_textures.size(), MAX_PAGES])
	assert(atlas_textures.size() <= MAX_PAGES, "Mehr Atlasseiten als der Shader Sampler hat")
	atlas_texture = atlas_textures[0]
	_width = width
	_height = height
	_frames = meta["frames"]
	...
	var table_width := mini(frame_count, TABLE_WIDTH)
	var table_height := int(ceil(float(frame_count) / table_width)) * 2
	var table := Image.create(table_width, table_height, false, Image.FORMAT_RGBA8)
	for i in frame_count:
		...
		var w: int = int(f["w"])
		var h: int = int(f["h"])
		assert(w <= 255 and h <= 255, "Frame größer als 255 Pixel — passt nicht in ein Byte")
		table.set_pixel(i % table_width, (i / table_width) * 2,
			Color8(x & 255, (x >> 8) & 255, y & 255, (y >> 8) & 255))
		table.set_pixel(i % table_width, (i / table_width) * 2 + 1,
			Color8(w, h, int(f.get("page", 0)), 0))
		var name: String = f["sprite"]
		if not sprite_first_frame.has(name):
			sprite_first_frame[name] = i
			sprite_frame_count[name] = 0
			sprite_frame_size[name] = Vector2i(w, h)
...
func make_rgba_texture(sprite: String, frame: int, player_row: int = 0) -> ImageTexture:
	...
	var page_raw: PackedByteArray = _page_raws[int(f.get("page", 0))]
	var w: int = f["w"]
	var h: int = f["h"]
...
func bind_atlases(mat: ShaderMaterial) -> void:
	for i in MAX_PAGES:
		mat.set_shader_parameter("atlas%d" % i, atlas_textures[mini(i, atlas_textures.size() - 1)])


static func custom_data(frame: int, player_row: int) -> Color:
	return Color(float(frame & 255) / 255.0, float((frame >> 8) & 255) / 255.0,
		float(player_row + 1) / 255.0, 0.0)

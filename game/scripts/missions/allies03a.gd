...
func send_ussr_water_transport() -> void:
	api.reinforce_with_transport(ussr, "lst", TRANSPORT_REINFORCEMENTS,
			[_loc("WaterTransportEntry"), _loc("WaterTransportLoadout")],
			[_loc("WaterTransportExit")], Callable(), Callable(), 3, func(u): api.hunt(u))

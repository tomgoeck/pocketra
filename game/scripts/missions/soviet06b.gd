...
func world_loaded() -> void:
	...
	goal_cells = _line([[85, 10], [85, 11], [85, 12], [86, 13], [87, 13], [88, 13], [88, 14],
		[89, 14], [90, 14], [90, 15], [91, 15], [91, 16], [91, 17], [92, 17], [93, 17], [94, 17],
		[94, 18], [95, 18], [96, 18], [96, 19], [97, 19], [98, 19]])
	base_blueprints = [
		{"type": "apwr", "name": "Apwr", "cost": 500, "shape": Vector2i(3, 3), "location": Vector2i(76, 41)},
		{"type": "tent", "name": "Tent", "cost": 400, "shape": Vector2i(2, 3), "location": Vector2i(67, 45), "on_built": "infantry"},
		{"type": "proc", "name": "Proc", "cost": 1400, "shape": Vector2i(3, 4), "location": Vector2i(71, 41)},
		{"type": "weap", "name": "Weap", "cost": 2000, "shape": Vector2i(3, 3), "location": Vector2i(67, 41), "on_built": "armor"},
		{"type": "apwr", "name": "Apwr2", "cost": 500, "shape": Vector2i(3, 3), "location": Vector2i(77, 45)},
	]
...
func order_response_cruiser() -> void:
	...
	if api.is_dead(cruiser):
		return
	api.attack_move(cruiser, _loc("waypoint94"))
	api.after_delay(api.seconds(90), func():
		if api.is_dead(cruiser):
			return
		api.on_idle(cruiser, func(u): api.attack_move(u, _loc("waypoint69"))))
...
func order_bridge_breakers(target, reveal: bool) -> void:
	if api.is_dead(target):
		return
	for n in ["BridgeBreaker", "BridgeBreaker2"]:
		var b = api.actor(n)
		if api.is_dead(b):
			continue
		api.stop(b)

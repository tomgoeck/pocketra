...
const FREEWARE_WEB_PATH := "content/ra-quickinstall.zip"
const SOURCES_FILE := "res://data/content_sources.json"
...
static func single_threaded() -> bool:
	if OS.get_cmdline_user_args().has("--force-web-content"):
		return true
	return web()


static func cd_downloads_supported() -> bool:
	return not web()


static func freeware_download_supported() -> bool:
	return not web() or not _configured_sources("web").is_empty()


static func _configured_sources(key: String) -> Array[String]:
	var out: Array[String] = []
	if not FileAccess.file_exists(SOURCES_FILE):
		return out
	var d = JSON.parse_string(FileAccess.get_file_as_string(SOURCES_FILE))
	if d is Dictionary:
		for v in d.get(key, []):
...
static func freeware_sources() -> Array[String]:
	var out: Array[String] = []
	if web():
		out.append_array(_configured_sources("web"))
		var base := WebFiles.page_dir_url()
		if base != "":
			out.append(base + FREEWARE_WEB_PATH)


		return out
	out.append_array(_configured_sources("native"))
	out.append(FREEWARE_URL)
	return out
...
static func free_space_bytes(target_dir: String) -> int:
	...
	if web():
		var est := WebFiles.storage_estimate()
		var quota := int(est.get("quota", -1))
		var usage := int(est.get("usage", -1))
		if quota <= 0:
			return -1
		return maxi(0, quota - maxi(0, usage))

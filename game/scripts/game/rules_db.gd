...
const IMPACT_TERRAIN := ["Ground", "Water", "Air"]
...
const SMUDGE_SCORCH := 1
const SMUDGE_CRATER := 2

const SMUDGE_SPRITES := {SMUDGE_SCORCH: ["sc1", "sc2", "sc3", "sc4", "sc5", "sc6"],
		SMUDGE_CRATER: ["cr1", "cr2", "cr3", "cr4", "cr5", "cr6"]}
...
func build(atlas: PaletteAtlas, overrides: Dictionary = {}, campaign: bool = false,
	...
	for name in actors:
		_mark_hits_allies(str(actors[name].get("death_weapon", "")))
...
func _mark_hits_allies(name: String) -> void:
	var seen := {}
	var w := name
	while w != "" and weapons.has(w) and not seen.has(w):
		seen[w] = true
		weapons[w]["hits_allies"] = true
		w = str(weapons[w].get("cluster_weapon", ""))
...
func _seq_frames(seq: Dictionary, body: String) -> Array:
	if seq.is_empty():
		return []
	var stem := _stem(str(seq.get("file", body + ".shp")))
	if not _has_sprite(stem):
...
func effect_key(image: String, seq: String, owner_palette: bool = false) -> String:
	...
	if length < 0:
		length = n - start
	effects[key] = _effect_entry(stem, start, maxi(1, mini(length, n - start)), e, owner_palette)
	return key


func _effect_entry(stem: String, start: int, length: int, e: Dictionary, owner_palette: bool) -> Dictionary:
	var frames: Array = []
	var n: int = _atlas.sprite_frame_count[stem]
	...
	return {"sprite": stem, "start": start, "length": length,
			"ticks": maxi(1, int(round(float(e.get("tick", 40)) / 40.0))),
			"offset_x": int(off[0]) if off.size() > 0 else 0,
			"offset_y": int(off[1]) if off.size() > 1 else 0,

			"zoffset": int(e.get("zoffset", 0)),
			"flipx": bool(e.get("flipx", false)),
			"owner_palette": owner_palette,
			"frames": frames}


const DAMAGE_WARHEADS := {"SpreadDamage": true, "TargetDamage": true, "HealthPercentageDamage": true}
...
func _build_weapons(src: Dictionary) -> void:
	for name in src:
		...
		for wh in w.get("warheads", []):
			if not DAMAGE_WARHEADS.has(wh["type"]):
				continue
			if dmg.is_empty():
				dmg = wh
		...
		for wh in w.get("warheads", []):
			...
			if keys.is_empty() and snd.is_empty():
				continue
			if impact_sound == "" and not snd.is_empty():
				impact_sound = snd[0]
			impacts.append({
				"effects": keys,
				"sound": snd[0] if not snd.is_empty() else "",
				"sound_chance": int(wh.get("impact_sound_chance", 100)),
				"impact_actors": bool(wh.get("impact_actors", true)),
				"delay": int(wh.get("delay", 0)),
				"valid_targets": _mask(wh.get("valid_targets", []), TARGET_TYPES),
				"invalid_targets": _mask(wh.get("invalid_targets", []), TARGET_TYPES),
				"valid_terrain": _mask(wh.get("valid_targets", []), IMPACT_TERRAIN),
				"invalid_terrain": _mask(wh.get("invalid_targets", []), IMPACT_TERRAIN),
			})
		...
		var smudge_type := 0
		var smudge_size := 0
		var smudge_size_inner := 0
		var smudge_chance := 100
		var smudge_valid := 0
		var smudge_invalid := 0
		for wh in w.get("warheads", []):
			if wh["type"] != "LeaveSmudge" or smudge_type != 0:
				continue
			for st in wh.get("smudge_types", []):
				if String(st).to_lower() == "scorch":
					smudge_type = SMUDGE_SCORCH
					break
				if String(st).to_lower() == "crater":
					smudge_type = SMUDGE_CRATER
					break
			if smudge_type == 0:
				continue
			smudge_size = int(wh.get("size", 0))
			smudge_size_inner = int(wh.get("size_inner", 0))
			smudge_chance = int(wh.get("chance", 100))
			smudge_valid = _mask(wh.get("valid_targets", []), TARGET_TYPES)
			smudge_invalid = _mask(wh.get("invalid_targets", []), TARGET_TYPES)
		...
		var percent: bool = String(dmg.get("type", "")) == "HealthPercentageDamage"
		var falloff: Array = dmg.get("falloff", [])
		if falloff.is_empty():
			falloff = [100, 100] if percent else [100, 37, 14, 5, 0]
		...
		weapons[name] = {
			...
			"damage": damage, "damage_percent": percent,
			"spread": maxi(1, int(dmg.get("spread", 128))), "versus": versus,
			"falloff": falloff,
			...
			"smudge_type": smudge_type, "smudge_size": smudge_size,
			"smudge_size_inner": smudge_size_inner, "smudge_chance": smudge_chance,
			"smudge_valid_targets": smudge_valid, "smudge_invalid_targets": smudge_invalid,
...
func _rotors(a: Dictionary, seqs: Dictionary) -> Array:
	...
	for ov in a.get("idle_overlays", []):
		...
		if req.begins_with("!airborne"):
			...
			air[key] = ov
		elif req == "" and str(ov.get("image", "")) == "" and not a.get("husk", false):
			air[key] = ov
			ground[key] = ov
	var out: Array = []
	for key in air:
...
func _rotor_seq(seqs: Dictionary, name: String) -> Dictionary:
	...
	return {"sprite": stem, "start": int(sq.get("start", 0)), "len": maxi(1, int(sq.get("length", 1))),
			...
			"off_x": so.x, "off_y": so.y}


func _death_effect(actor: String, suffix: String, sq: Dictionary, body: String, owner_palette: bool) -> String:
	if sq.is_empty():
		return ""
	...
	if not effects.has(key):
		effects[key] = _effect_entry(stem, start, length, sq, owner_palette)
	return key
...
func _build_type(name: String, a: Dictionary) -> Dictionary:
	...
	var t := {
		...
		"interactable_only": a.get("interactable_only", false),
		...
		"z_offset": int(idle.get("zoffset", 0)),
		...
		"infiltrates_ally": _mask(a.get("infiltrates_ally", []), TARGET_TYPES),
		"infil_transform": _mask(a.get("infil_transform", []), TARGET_TYPES),
		"capture_health": int(a.get("capture_health", 0)),
		...
		"infil_cash_max": int(a.get("infil_cash_max", 2147483647)),
		...
		"infil_reset": _mask(a.get("infil_reset", []), TARGET_TYPES),
		...
		"reveal_gap_range": int(a.get("reveals_gap", a.get("reveals", 0))),

		"creates_shroud_range": int(a.get("creates_shroud", 0)),

		"jammer_range": int(a.get("jammer_range", 0)),
	...
	if not b.is_empty():
		...
		if queue >= 0 and int(a.get("cost", 0)) > 0:
			...
			t["at_production"] = String(b.get("at_production", ""))
	...
	if weapon != "":
		...
		var turret_names: Array = []
		for tu in a.get("turrets", []):
			turret_names.append(str(tu.get("name", "primary")))
		var slots: Array = []
		...
		for arm in a.get("armaments", []):
			...
			if w2 == "" or not weapons.has(w2):
				continue
			var ti := turret_names.find(str(arm.get("turret", "primary")))
			var entry: Array = [w2, 0 if ti < 0 else ti]
			if not primary_used and w2 == weapon:
				primary_used = true
				slots.insert(0, entry)
			else:
				slots.append(entry)
		if slots.size() > 1:
			t["weapon_secondary"] = str(slots[1][0])
		if slots.size() > 2:
			t["weapon_tertiary"] = str(slots[2][0])
		if turret_names.size() > 1:
			var arm_turrets: Array = []
			for k in range(min(slots.size(), 3)):
				arm_turrets.append(int(slots[k][1]))
			if not arm_turrets.is_empty():
				t["arm_turrets"] = arm_turrets
	...
	if a.has("turret"):
		...
		var toff: Array = []
		for tu in a.get("turrets", []):
			var o: Array = tu.get("offset", [0, 0, 0])
			toff.append([int(o[0]) if o.size() > 0 else 0, int(o[1]) if o.size() > 1 else 0,
					int(o[2]) if o.size() > 2 else 0])
		if toff.size() > 1:
			t["turret_offsets"] = toff
		...
		if not ts.is_empty():
			...
			if _has_sprite(tstem):
				...
				var tsize: Vector2i = _atlas.sprite_frame_size[tstem]
				t["turret_w"] = tsize.x
				t["turret_h"] = tsize.y
				var tseq := _seq_offset(ts)
				t["turret_off_x"] = tseq.x
				t["turret_off_y"] = tseq.y


				t["turret_facings"] = maxi(1, int(ts.get("facings", 32)))
				t["turret_classic"] = bool(ts.get("classic", false))
	...
	if a.has("initial_stance"):
		t["initial_stance"] = STANCES.find(str(a["initial_stance"]))
	if a.has("initial_stance_ai"):
		t["initial_stance_ai"] = STANCES.find(str(a["initial_stance_ai"]))
	...
	if int(a.get("cargo", 0)) > 0:
		...
		var cd: Array = a.get("cargo_delays", [8, 0, 25, 8])
		...
		t["after_load_delay"] = int(cd[3]) if cd.size() > 3 else 8
	...
	if not a.get("landing_craft", []).is_empty():
		var ramp_open: Array = _seq_frames(seqs.get("open", {}), body)
		if not ramp_open.is_empty():
			t["ramp_terrain"] = _mask(a["landing_craft"], TERRAIN_NAMES)
			t["ramp_open"] = ramp_open
			var ramp_close: Array = _seq_frames(seqs.get("close", {}), body)
			if not ramp_close.is_empty():
				t["ramp_close"] = ramp_close
			var hold: Array = _seq_frames(seqs.get("unload", {}), body)
			if not hold.is_empty():
				t["ramp_hold"] = int(hold[0])
			var otick := int(seqs.get("open", {}).get("tick", 40))
			t["ramp_ticks"] = maxi(1, ramp_open.size() * maxi(1, otick) / 40)


	if a.get("with_muzzle_overlay", false):
		for arm in a.get("armaments", []):
			var mseq: String = str(arm.get("muzzle_sequence", ""))
			if mseq == "" or arm.get("requires", "") != "":
				continue
			var msq: Dictionary = seqs.get(mseq, {})
			if msq.is_empty():
				continue
			var mstem := _stem(str(msq.get("file", "")))
			if mstem == "" or not _has_sprite(mstem):
				continue
			var mlen: int = int(msq.get("length", 1))
			if mlen < 0:
				mlen = _atlas.sprite_frame_count[mstem] - int(msq.get("start", 0))
			t["muzzle_sprite"] = mstem
			t["muzzle_start"] = int(msq.get("start", 0))
			t["muzzle_len"] = maxi(1, mlen)

			var lo: Array = []
			for part in str(arm.get("local_offset", "")).split(","):
				if part.strip_edges() != "":
					lo.append(int(part.strip_edges()))
			if a.get("muzzle_ignore_offset", false):
				lo = []
			t["muzzle_ox"] = int(lo[0]) if lo.size() > 0 else 0
			t["muzzle_oy"] = int(lo[1]) if lo.size() > 1 else 0
			t["muzzle_oz"] = int(lo[2]) if lo.size() > 2 else 0

			t["muzzle_turret"] = a.has("turret") and str(arm.get("turret", "primary")) != ""
			break
	...
	if a.get("husk", false):
		t["husk"] = true
		t["husk_terrain"] = _mask(a.get("husk_terrain", []), TERRAIN_NAMES)


	if a.get("husk", false) and not a.has("aircraft"):


		var hloco: String = str(a.get("husk_locomotor", "tracked"))
		t["locomotor"] = hloco
		t["crushes"] = _mask(LOCO_CRUSHES.get(hloco, []), CRUSH_CLASSES)


		t["facings"] = int(idle.get("facings", 1))
		t["classic"] = idle.get("classic", false) or t["facings"] == 32

		var hov: Dictionary = a.get("husk_overlay", {})
		if not hov.is_empty():
			var bstem := _stem(str(hov.get("file", "fire1.shp")))
			if _has_sprite(bstem):
				var blen: int = int(hov.get("length", -1))
				if blen < 0:
					blen = _atlas.sprite_frame_count[bstem] - int(hov.get("start", 0))
				t["burn_sprite"] = bstem
				t["burn_start"] = int(hov.get("start", 0))
				t["burn_len"] = maxi(1, blen)

				t["burn_ticks"] = maxi(1, int(hov.get("tick", 40)) / 40)
				var boff := _seq_offset(hov)
				t["burn_off_x"] = boff.x
				t["burn_off_y"] = boff.y


	if a.has("falls_to_earth"):
		var fte: Dictionary = a["falls_to_earth"]
		t["falls_to_earth"] = true
		t["fall_moves"] = bool(fte.get("moves", false))
		t["fall_velocity"] = maxi(1, int(fte.get("velocity", 43)))
		t["fall_max_spin"] = int(fte.get("max_spin", -1))
		t["fall_weapon"] = String(fte.get("explosion", ""))
	if a.has("spawn_on_death"):
		var sod: Dictionary = a["spawn_on_death"]
		t["husk_actor"] = String(sod.get("actor", ""))
		t["husk_probability"] = int(sod.get("probability", 100))


	if a.has("capture_into"):
		t["capture_into"] = String(a["capture_into"])
	...
	if a.has("cloak") and typeof(a["cloak"]) == TYPE_DICTIONARY:
		...
		t["cloak_pause_critical"] = bool(ck.get("pause_critical", false))
		if str(ck.get("sound", "")) != "":
			t["cloak_sound_name"] = str(ck["sound"])
	if a.has("detect_cloaked"):
		...
		t["render_detect_circle"] = bool(a["detect_cloaked"].get("render_circle", false))
	if a.has("crushable"):
	...
	var has_death_weapon: bool = dw != "" and weapons.has(dw)
	if has_death_weapon:
		t["death_weapon"] = dw
	if dw != "" and _weapon_defs.has(dw):
		...
		for wh in wd.get("warheads", []):
			if wh["type"] == "CreateEffect":
				...
				if not ex.is_empty() and death_any == "" and not has_death_weapon:
	...
	if a.has("death_animation"):
		...
		for i in DEATH_TYPES.size():
			...
			var key := _death_effect(name, "die%d" % idx if suffix else "dead", seqs.get(sname, {}), body, true)
			if suffix and idx <= 0:
				key = ""
			if key == "":
				key = death_any
		...
		death_effects.append(_death_effect(name, "crushed", seqs.get(str(da.get("crushed", "")), {}), body, false))
	else:
	...
	if a.has("building"):
		...
		t["requires_base_provider"] = bd.get("requires_base_provider", false)
		...
		if a.has("produces"):
			...
			t["produces_types"] = a["produces"].duplicate()


		if a.get("reservable", false):
			t["reservable"] = true
		...
		if a.has("repairable"):
			...
			var rep: Array = a["repairable"]
			if rep is Array and not rep.is_empty():
				t["repair_actors"] = rep.duplicate()
		if a.has("transforms"):
		...
		if a.has("mad_tank"):
			var mt: Dictionary = a["mad_tank"]
			t["mad_charge_delay"] = int(mt.get("charge_delay", 96))
			t["mad_detonation_delay"] = int(mt.get("detonation_delay", 42))
			t["mad_thump_interval"] = maxi(1, int(mt.get("thump_interval", 8)))
			t["mad_thump_weapon"] = String(mt.get("thump_weapon", ""))
			t["mad_detonation_weapon"] = String(mt.get("detonation_weapon", ""))
			t["mad_charge_sound"] = String(mt.get("charge_sound", ""))
			t["mad_detonation_sound"] = String(mt.get("detonation_sound", ""))
			t["mad_driver"] = String(mt.get("driver", "e1"))

			var piston: Dictionary = seqs.get(String(mt.get("thump_sequence", "piston")), {})
			_add_facing_frame_list(t, "mad_frames", "mad_len", piston, body)
		if a.get("detonate_on_deploy", false):
			t["detonate_on_deploy"] = true
		if a.has("portable_chrono"):
			var pc: Dictionary = a["portable_chrono"]
			t["chrono_charge_delay"] = int(pc.get("charge_delay", 500))
			t["chrono_max_distance"] = int(pc.get("max_distance", 12))
			t["chrono_sound"] = String(pc.get("sound", "chrotnk1"))
	elif a.has("aircraft"):
		...
		t["repulsable"] = ac.get("repulsable", true)
		t["repulsion_speed"] = int(ac.get("repulsion_speed", -1))
		t["ideal_separation"] = int(ac.get("ideal_separation", 1706))
	...
	if not t.has("rotors"):
		var idle_overlays := _rotors(a, seqs)
		if not idle_overlays.is_empty():
			t["rotors"] = idle_overlays
	return t

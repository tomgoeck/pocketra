...
func _ready() -> void:
	...
	visibility_changed.connect(func(): if visible: _auto_ready = -1)
...
func _all_types(k: int) -> Array:
	...
	for name in world.types:
		...
		for p in t.get("prerequisites", []):
			if not _token_possible(str(p), faction):
				ok = false
		if not ok:
			continue
...
func _token_possible(token: String, faction: String) -> bool:
...
func _missing_text(type_id: int) -> String:
	...
	for p in t.get("prerequisites", []):
		...
		if token.begins_with("techlevel.") or token == "":
			continue
		if world.sim.has_prerequisite(0, token):
			continue
	...
	for name in world.types:
		...
		for i in provides.size():
			...
			for p2 in t.get("prerequisites", []):
				if not _token_possible(str(p2), faction):
					own = false
			if own:
				return Names.of(t)
			if fallback == "":
...
func _rebuild() -> void:
	...
	if sim.has_method("hidden_items"):
		var hidden := {}
		for t in sim.hidden_items(0, kind):
			hidden[t] = true
	...
	var ready := {}
	for t in sim.buildable(0, kind):
		ready[t] = true
	var queue: PackedInt32Array = sim.queue_state(0, kind)
	var counts := {}
	var progress := {}
	var done := {}
	var seconds := {}
	...
	if ready_type != _auto_ready:
		...
		if ready_type >= 0 and visible and world.placing_type < 0:
	...
	for vi in visible_tabs.size():
		...
		_tabs[i].position = Vector2(margin + vi * (tab_w + margin), margin)
	...
	for i in _slots.size():
		...
		s.limited = sim.has_method("build_limit_reached") and sim.build_limit_reached(0, t)
...
func _update_ready_glow(sim) -> void:
	var any := false
	for i in _tabs.size():
		var q: PackedInt32Array = sim.queue_state(0, KIND_QUEUES[i])
...
func _on_hold(s: BuildSlot) -> void:
	...
	if world.sim.has_method("pause_build"):
		...
		hold.pressed.connect(func():
			world.sim.pause_build(0, kind, not _paused)
			world.sfx.play_ui("onhold1")
			_close_popup()
			_refresh = 1.0)
		box.add_child(hold)

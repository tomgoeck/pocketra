...
func _on_tap(s: BuildSlot) -> void:
	...
	if not s.available:
		...
		if HomepageDemo.enabled() and HomepageDemo.locked(world.type_names[s.type_id],
				world.types[world.type_names[s.type_id]]):
			blocked.emit("%s — %s" % [label, "nur in der Vollversion" if Lang.is_de() else "full version only"])
			return
		if s.limited:

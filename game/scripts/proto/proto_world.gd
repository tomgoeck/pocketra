...
const AI_LEVELS := {
	"easy": {
		...
		"harv_stall_ticks": 1500, "harvesters_extra": 0,


		"straggler_min_cells": 5, "straggler_interval": 250, "gather_ready_percent": 80,
		"gather_repicks_max": 1, "no_flee_ticks": 200, "counter_push_on": 0,


		"defense_aa_max": 2, "defense_army_percent": 15, "defense_growth_tick": 0,


		"air_squad_size": 3, "air_soft_targets": 0,
		"naval_escort": 0, "naval_hunt_cloaked": 0,


		"naval_unit_limit": 2, "naval_escort_per_bombard": 2,
		"continuous_production": 1, "wave_value": 0,


		"opening_plan": 1, "opening_refineries": 3, "opening_factory_after": 3,
		"opening_production_free": 1, "opening_defense_per": 3, "opening_attack_ticks": 750,
	},
	"normal": {
		...
		"harv_stall_ticks": 800, "harvesters_extra": 1,
		"straggler_min_cells": 5, "straggler_interval": 150, "gather_ready_percent": 80,
		"gather_repicks_max": 1, "no_flee_ticks": 300, "counter_push_on": 0,
		"defense_aa_max": 3, "defense_army_percent": 30, "defense_growth_tick": 12000,


		"air_squad_size": 2, "air_soft_targets": 1,
		"naval_escort": 1, "naval_hunt_cloaked": 0,

		"naval_unit_limit": 4, "naval_escort_per_bombard": 2,
		"continuous_production": 1, "wave_value": 4500,
	...
	"hard": {
		...
		"gather_max_ticks": 300, "regen_ticks": 500,
		"min_first_attack_tick": 0,
		"allow_raid": 1, "allow_siege": 1, "allow_support_powers": 1,
		...
		"harv_stall_ticks": 400, "harvesters_extra": 1,


		"straggler_min_cells": 5, "straggler_interval": 100, "gather_ready_percent": 60,
		"gather_repicks_max": 1, "no_flee_ticks": 300, "counter_push_on": 1,


		"defense_aa_max": 4, "defense_army_percent": 45, "defense_growth_tick": 6000,


		"air_squad_size": 1, "air_soft_targets": 1,
		"naval_escort": 1, "naval_hunt_cloaked": 1,
		...
		"opening_plan": 1, "opening_refineries": 3, "opening_factory_after": 2,
		"opening_production_free": 2, "opening_defense_per": 4, "opening_attack_ticks": 750,
	},
}
...
func _ready() -> void:
	...
	if HomepageDemo.enabled():
		HomepageDemo.configure()
		map_data = HomepageDemo.make_map()
	else:
...
func _build_terrain(terrain: PaletteAtlas) -> void:
	...
	for y in range(bounds.position.y, bounds.end.y):
		for x in range(bounds.position.x, bounds.end.x):
			...
			mm.set_instance_custom_data(i, PaletteAtlas.custom_data(terrain.frame_index(image, ti % maxi(n, 1)), terrain.terrain_row))
			i += 1
...
func _set_bridge_template(b: Dictionary, tpl: int) -> void:
	...
	for c: Vector2i in b["cells"]:
		...
		if _terrain_mm != null and bounds.has_point(c):
			...
			_terrain_mm.set_instance_custom_data(i, PaletteAtlas.custom_data(_terrain.frame_index(image, ind % frames), _terrain.terrain_row))
	terrain_version += 1
...
func _start_sim() -> void:
	...
	for name in order:
		...
		for key in ["footprint", "sprite_h", "refinery", "dock_dx", "dock_dy", "dock_angle", "damaged_frame", "idle_len",
					...
					"repulsable", "repulsion_speed", "ideal_separation",
		...
		if HomepageDemo.enabled() and HomepageDemo.locked(name, t):
			def["prerequisites"] = t.get("prerequisites", []).duplicate()
			def["prerequisites"].append("homepage.full_version_only")
...
func _diag_tick(delta: float) -> void:
	...
	if _diag_probe == null:
		...
		mm.set_instance_custom_data(0, PaletteAtlas.custom_data(_atlas.frame_index("fact", 0) if _atlas.sprite_frame_count.has("fact") else 0, 0))
		mm.set_instance_transform_2d(1, Transform2D(0.0, at + Vector2(60, 0)))
		mm.set_instance_custom_data(1, PaletteAtlas.custom_data(_atlas.frame_index("mcv", 0) if _atlas.sprite_frame_count.has("mcv") else 0, 1))
...
func _rebuild_smudges() -> void:
	...
	for i in instances.size():
		...
		_smudge_mm.set_instance_custom_data(i, PaletteAtlas.custom_data(instances[i][1], _terrain.terrain_row))
...
func _rebuild_ore() -> void:
	...
	for i in instances.size():
		...
		_ore_mm.set_instance_custom_data(i, PaletteAtlas.custom_data(instances[i][1], _terrain.terrain_row))

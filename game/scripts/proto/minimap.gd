...
func _process(delta: float) -> void:
	if world != null:
		...
		var jam: bool = world.radar_jammed() if world.has_method("radar_jammed") else false
		if on != _radar_on or jam != _jammed:
			_radar_on = on
			_jammed = jam
			_switch_t = 0.0
...
func _noise_texture() -> ImageTexture:
	var step := int(Time.get_ticks_msec() / NOISE_MS)
	if _noise_tex == null:
		_noise_img = Image.create(RADAR_FRAME_W, RADAR_FRAME_H, false, Image.FORMAT_RGBA8)
		_noise_tex = ImageTexture.create_from_image(_noise_img)
		_noise_step = -1
	if step == _noise_step:
		return _noise_tex
	_noise_step = step
	var img := Image.create(NOISE_W, NOISE_H, false, Image.FORMAT_RGBA8)

	var rs := (step * 1103515245 + 12345) & 0x7fffffff
	var band := NOISE_H - 1 - (step % (NOISE_H + 8))
	for y in NOISE_H:

		rs = (rs * 1103515245 + 12345) & 0x7fffffff
		var row := 0.55 + 0.45 * float((rs >> 12) & 0xff) / 255.0
		if absi(y - band) <= 1:
			row = 1.6
		for x in NOISE_W:
			rs = (rs * 1103515245 + 12345) & 0x7fffffff
			var v: float = clampf(float((rs >> 16) & 0xff) / 255.0 * row, 0.0, 1.0)
			img.set_pixel(x, y, Color(v * 0.32, v * 0.78, v * 0.30, 1.0))
	img.resize(RADAR_FRAME_W, RADAR_FRAME_H, Image.INTERPOLATE_NEAREST)
	_noise_img = img
	_noise_tex.update(img)
	return _noise_tex


func _draw_radar_jammed() -> void:
	var rect := Rect2(Vector2.ZERO, size)
	draw_rect(rect, Color(0.02, 0.03, 0.02, 1.0), true)

	var a := clampf(_switch_t / (SWITCH_SECONDS * 0.5), 0.25, 1.0)
	draw_texture_rect(_noise_texture(), rect, false, Color(1, 1, 1, a))
	draw_texture_rect(_scanlines(), rect, true, Color(1, 1, 1, 1))
	draw_texture_rect(_vignette(), rect, false)
...
func _draw() -> void:
	...
	for u in world.units:
		...
		if not u.alive or not u.visible or t.get("decoration", false):
			continue
		if not vis_ready and u.player != world.local_player:
			continue
	...
	for d in world.gps_dots():
		var gp: Vector2 = ((d[0] as Vector2) - r.position) * k
		var gc: Color = world.player_colors[d[1]] if d[1] < world.player_colors.size() else Color.WHITE
		if d[2]:
			draw_rect(Rect2(gp - Vector2(dot, dot) / 2.0, Vector2(dot, dot)), gc)
		else:
			draw_circle(gp, dot * 0.5, gc)

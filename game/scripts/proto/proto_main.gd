...
func _touch_model() -> bool:
	return _force_touch or not (Desktop.is_desktop() or _mouse_model())


func _mouse_model() -> bool:
	return Desktop.has_keyboard() and _tap_from_mouse and not _force_touch
...
func _ready() -> void:
	...
	_test_air_spread = args.has("--test-air-spread")
	...
	for a in args:
		...
		if a == "--mp-autoplay" or a == "--mp-shot":
			_any_test = true
	var astk := args.find("--air-spread-type")
	if astk >= 0 and astk + 1 < args.size():
		_spread_type = String(args[astk + 1])
	...
	gestures.tap.connect(_on_tap_gesture)
	...
	if not GestureHelp.seen() and _screenshot_path == "" and not _any_test and not HomepageDemo.enabled():
...
func _on_tap_gesture(p: Vector2) -> void:
	if world == null or not _mouse_model():
		_on_tap(p)
		return
	...
	world.queue_orders = shift
	world.additive_select = add_before or shift
	_on_tap(p)
...
var _test_air_spread := false
var _spread_units: Array = []
var _spread_min := -1.0
var _spread_min_move := -1.0
var _spread_close := 0
var _spread_ticks := 0
var _spread_last := -1
var _spread_foe = null
var _spread_type := ""
...
func _run_test_air_spread() -> void:
	var sim = world.sim
	var tick: int = sim.tick()
	var soviet := world.player_faction() == "soviet"
	var tn := _spread_type
	if tn == "":
		tn = "hind" if soviet else "heli"
	if _test_step == 0 and tick >= 10:
		var origin := Vector2i(24, 24)
		for u in world.units:
			if u.alive and u.player == 0:
				origin = Vector2i(u.pos / ProtoWorld.CELL)
				break
		...
		_spread_units.clear()
		if not world.type_ids.has(tn):
			print("T: --test-air-spread: kein Typ ", tn)
			get_tree().quit()
			return

		var ring := [Vector2i(-3, -3), Vector2i(0, -4), Vector2i(3, -3),
				Vector2i(-3, 3), Vector2i(0, 4), Vector2i(3, 3)]
		for off in ring:
			var u = world.spawn_unit(tn, 0, origin + off, 0, 100, true)
			if u != null:
				_spread_units.append(u)

		_spread_foe = world.spawn_unit("2tnk", 1, origin + Vector2i(18, 18))
		var d: Dictionary = world.types[tn]
		print("T%d --test-air-spread: %d × %s, repulsable=%s repulsion_speed=%s ideal_separation=%s" %
				[tick, _spread_units.size(), tn, d.get("repulsable", "?"),
				d.get("repulsion_speed", "?"), d.get("ideal_separation", "?")])
		world.center_on((Vector2(origin) + Vector2(6, 6)) * ProtoWorld.CELL)
		_test_step = 1
		_test_tick = tick
	elif _test_step == 1 and tick >= _test_tick + 40:


		world.select_units(_spread_units)
		world.order_move((Vector2(_test_origin) + Vector2(14, 14)) * ProtoWorld.CELL)
		_spread_min_move = -1.0
		_test_step = 2
		_test_tick = tick
	elif _test_step == 2:
		var d: float = _spread_pair_min()
		if d >= 0.0 and tick >= _test_tick + 120 and tick != _spread_last:
			_spread_last = tick
			_spread_ticks += 1
			if d < 512.0:
				_spread_close += 1
			if _spread_min_move < 0.0 or d < _spread_min_move:
				_spread_min_move = d
		if tick < _test_tick + 400:
			return
		print("T%d Fahrbefehl auf einen Punkt: kleinster paarweiser Abstand %.0f Welteinheiten (%.2f Zellen), am Ende %.0f, unter einer halben Zelle in %d von %d Ticks" %
				[tick, _spread_min_move, _spread_min_move / 1024.0, _spread_pair_min(), _spread_close, _spread_ticks])

		if _spread_foe != null and _spread_foe.alive:
			world.select_units(_spread_units)
			world.order_attack(_spread_foe)
		_spread_min = -1.0
		_spread_close = 0
		_spread_ticks = 0
		_test_step = 3
		...
		if d >= 0.0 and tick >= _test_tick + 120 and tick != _spread_last:
			...
			if d < 512.0:
				_spread_close += 1
			if _spread_min < 0.0 or d < _spread_min:
				_spread_min = d
		if tick < _test_tick + 400:
			return
		var alive := 0
		for u in _spread_units:
			if u.alive:
				alive += 1
		print("T%d Angriffsbefehl auf dasselbe Ziel: kleinster paarweiser Abstand %.0f Welteinheiten (%.2f Zellen), am Ende %.0f, unter einer halben Zelle in %d von %d Ticks, %d/%d in der Luft" %
				[tick, _spread_min, _spread_min / 1024.0, _spread_pair_min(), _spread_close, _spread_ticks, alive, _spread_units.size()])
		print("T: --test-air-spread fertig (IdealSeparation der Regeln: %s)" %
				str(world.types[tn].get("ideal_separation", "?")))
		if _screenshot_path != "":
			get_viewport().get_texture().get_image().save_png(_screenshot_path)
			print("Screenshot: ", _screenshot_path)
		get_tree().quit()
...
func _spread_pair_min() -> float:
	var best := -1.0
	for a in range(_spread_units.size()):
		var ua = _spread_units[a]
		if not ua.alive:
			continue
		for b in range(a + 1, _spread_units.size()):
			var ub = _spread_units[b]
			if not ub.alive:
				continue
			var d: float = ua.pos.distance_to(ub.pos) / float(ProtoWorld.CELL) * 1024.0
...
func _run_test_desktop_input() -> void:
	...
	if panzer == null or kamerad == null or gegner == null:
		...
		var l_b: bool = world.selection.size() == 1 and panzer.goal_kind == 0 \
			and panzer.goal.distance_to(leer_pos) < 3.0 * ProtoWorld.CELL
		fehler += 0 if l_b else 1
		sagen.call("Linksklick bewegt", l_b, " - Befehlsart %d (0=Bewegen), Ziel %.1f Zellen daneben, Auswahl %d" % [
		...
		await klick.call(MOUSE_BUTTON_LEFT, p_gegner)
		var l_c: bool = panzer.goal_kind == 2
		fehler += 0 if l_c else 1
		sagen.call("Linksklick greift an", l_c, " - Befehlsart %d (2=Angriff)" % panzer.goal_kind)
		...
		var ring_auf: bool = radial.visible and _radial_sticky
		var ring_befehle: bool = radial.ITEMS.has("force_fire") or radial.ITEMS.has("force_attack")
		await rechts.call(p_kamerad)
		var ring_zu: bool = not radial.visible
		var l_e: bool = ring_auf and ring_befehle and ring_zu and panzer.goal_kind == 0 \
			and world.selection.size() == 1 and world.selection[0].id == panzer.id
		fehler += 0 if l_e else 1
		sagen.call("Rechtsklick oeffnet den Ring", l_e, " - offen %s, Zwangsfeuer im Ring %s, zweiter Klick schliesst %s, kein Befehl %s, Auswahl bleibt %s" % [
			ring_auf, ring_befehle, ring_zu, panzer.goal_kind == 0,
			world.selection.size() == 1 and world.selection[0].id == panzer.id])
		...
		await klick.call(MOUSE_BUTTON_LEFT, stelle.call(ziel_a))
		await halten.call(KEY_SHIFT, true)
		await klick.call(MOUSE_BUTTON_LEFT, stelle.call(ziel_b))
		...
		sagen.call("Umschalt+Linksklick haengt an", l_f, " - Weg %.1f px (>0 = erst zum ersten Ziel), Fahne zurueckgesetzt %s" % [
		...
		await klick.call(MOUSE_BUTTON_LEFT, p_frei)
		await halten.call(KEY_CTRL, false)
		var l_g: bool = panzer.goal_kind == 2 and world.selection.size() == 1 \
			and world.selection[0].id == panzer.id
		fehler += 0 if l_g else 1
		sagen.call("Strg+Linksklick zwingt", l_g, " - Befehlsart %d (2=Angriff auf eigene Einheit), Auswahl bleibt %s" % [
			panzer.goal_kind, world.selection.size() == 1 and world.selection[0].id == panzer.id])


		world.select_only(panzer)
		await taste.call(KEY_ESCAPE)
		var esc_leer: bool = world.selection.is_empty() and not _menu_panel.visible
		await rechts.call(stelle.call(leer_pos))


		var l_h: bool = esc_leer and _order_mode == "" and _click_mode == "" and not _menu_panel.visible
		if radial.visible:
			_desktop_cancel_radial()
		fehler += 0 if l_h else 1
		sagen.call("Escape hebt auf, Rechtsklick ohne Auswahl befiehlt nicht", l_h,
			" - Auswahl leer %s, Pausenmenue zu %s, kein Modus %s" % [
			esc_leer, not _menu_panel.visible, _order_mode == ""])
...
const HARV_STATE_NAMES := ["IDLE", "SEARCH", "TO_FIELD", "HARVESTING", "TO_DOCK", "DOCK_TURN",
	"UNLOADING", "WAIT", "QUEUE", "EXPLORE", "PARKING", "PARKED"]
...
const AI_STAT_KEYS := ["gebaut", "superwaffen", "trupps", "erster_angriff", "armee", "bargeld",
	...
	"sammler_still_max", "sammler_still_spitze", "sammler_still_proben", "sammler_still_erz",
	"sammler_soll",
...
func _ai_stats(pl: int) -> Dictionary:
	...
	if sim.has_method("bot_stats"):
		...
		var zust: Array = d.get("sammler_still_zustand", [])
		var teile: PackedStringArray = []
		for k in mini(zust.size(), HARV_STATE_NAMES.size()):
			if int(zust[k]) > 0:
				teile.append("%s:%d" % [HARV_STATE_NAMES[k], int(zust[k])])
		out["sammler_still_zustand"] = "-" if teile.is_empty() else ",".join(teile)
		return out
...
func _print_tournament_rows(tick: int) -> void:
	...
	for e in world.player_roster:
		...
		print("TURNIER tick=%d pl=%d strategie=%s staerke=%s plan=%s gebaut=%d superwaffen=%d trupps=%d erster_angriff=%d lebend=%d ertrag=%d credits=%d spieler_lebend=%d ausgang=%d armee=%d bargeld=%d belagerer_verloren=%d in_turmreichweite=%d tuerme=%d trupp_verluste=%d schwere_verluste=%d sammler_still_max=%d sammler_still_spitze=%d sammler_still_proben=%d sammler_still_erz=%d sammler_soll=%d sammler_still_zustand=%s trupp_gather_ticks=%d trupp_marsch_ticks=%d trupp_kontakte=%d trupp_bis_schuss=%d trupp_ohne_schuss=%d sammel_neu=%d nachzuegler_stops=%d flaktuerme=%d turm_streuung=%d ungedeckt=%d luftangriffe=%d flieger_verloren=%d schiffe_verloren=%d schiffe_versenkt=%d gebaut_fahrzeuge=%d gebaut_infanterie=%d gebaut_luft=%d gebaut_schiffe=%d armee_land=%d armee_marine=%d t_raffinerie=%d t_kaserne=%d t_fabrik=%d t_radar=%d" % [
			...
			st["sammler_still_max"], st["sammler_still_spitze"], st["sammler_still_proben"],
			st["sammler_still_erz"], st["sammler_soll"], st["sammler_still_zustand"],
			st["trupp_gather_ticks"], st["trupp_marsch_ticks"], st["trupp_kontakte"],
			st["trupp_bis_schuss"], st["trupp_ohne_schuss"], st["sammel_neu"],
			st["nachzuegler_stops"],
			st["flaktuerme"], st["turm_streuung"], st["ungedeckt"],
			st["luftangriffe"], st["flieger_verloren"], st["schiffe_verloren"], st["schiffe_versenkt"],
			st["gebaut_fahrzeuge"], st["gebaut_infanterie"], st["gebaut_luft"], st["gebaut_schiffe"], st["armee_land"], st["armee_marine"],
			st["t_raffinerie"], st["t_kaserne"], st["t_fabrik"], st["t_radar"]])
...
func _run_test_ai() -> void:
	...
	if tick >= _test_tick + 1000:
		...
		for pl in per:
			...
			print("T%d KI%d [%s] luftbaubar=%s: %s credits=%d ertrag=%d squads=%d plan=%s armee=%d bargeld=%d belagerer_verloren=%d in_turmreichweite=%d tuerme=%d schwere_verluste=%d sammler_soll=%d trupp_gather_ticks=%d trupp_marsch_ticks=%d trupp_kontakte=%d trupp_bis_schuss=%d trupp_ohne_schuss=%d sammel_neu=%d nachzuegler_stops=%d flaktuerme=%d turm_streuung=%d ungedeckt=%d luftangriffe=%d flieger_verloren=%d schiffe_verloren=%d schiffe_versenkt=%d t_raffinerie=%d t_kaserne=%d t_fabrik=%d t_radar=%d" % [
				...
				st["in_turmreichweite"], st["tuerme"], st["schwere_verluste"], st["sammler_soll"],
				st["trupp_gather_ticks"], st["trupp_marsch_ticks"], st["trupp_kontakte"],
				st["trupp_bis_schuss"], st["trupp_ohne_schuss"], st["sammel_neu"],
				st["nachzuegler_stops"],
				st["flaktuerme"], st["turm_streuung"], st["ungedeckt"],
				st["luftangriffe"], st["flieger_verloren"], st["schiffe_verloren"],
				st["schiffe_versenkt"],
				st["t_raffinerie"], st["t_kaserne"], st["t_fabrik"], st["t_radar"]])


			var turm_liste: PackedStringArray = []
			for u in world.units:
				if u.alive and u.player == int(pl) and bool(world.types.get(u.type, {}).get("defense", false)):
					turm_liste.append("%s(%d,%d)" % [u.type, int(u.pos.x) / ProtoWorld.CELL, int(u.pos.y) / ProtoWorld.CELL])
			if not turm_liste.is_empty():
				print("   KI%d Türme: %s" % [pl, " ".join(turm_liste)])
...
const DEMO_PAUSE_GRID := [
	["menu.main.options", "_show_options"], ["pause.restart", "_restart_battle"],
]
...
func _rebuild_pause_menu() -> void:
	...
	for entry in alle:
		...
		if str(entry[0]) == "pause.quit_game" and OS.has_feature("web"):
			continue
...
func _desktop_mouse_button(mb: InputEventMouseButton) -> bool:
	...
	_tap_from_mouse = true
	if mb.pressed:
		if _radial_sticky or radial.visible:
			_desktop_cancel_radial()
			return true
		_rmb_down = true
...
func _process(delta: float) -> void:
	...
	if _test_crash and world.sim != null:
		_run_test_crash()
	if _test_air_spread and world.sim != null:
		_run_test_air_spread()

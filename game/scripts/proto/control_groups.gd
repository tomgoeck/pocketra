...
func layout_slots(max_height: float) -> void:
	slot_h = Dp.px(HudTheme.GROUP_BTN_H_DP)
	slot_w = Dp.px(HudTheme.GROUP_BTN_W_DP)
	var gap := Dp.px(HudTheme.GROUP_GAP_DP)
	step = maxf(slot_h + gap, HudTheme.min_touch_px())
	if max_height > 0.0:
		step = clampf(max_height / float(SLOTS), slot_h + gap, step)
	var w := maxf(slot_w, HudTheme.min_touch_px())
	size = Vector2(w, step * SLOTS)


	set_meta("hit_pad", Vector2((w - slot_w) / 2.0, (step - slot_h) / 2.0))


func _slot_at(p: Vector2) -> int:
	if p.x < 0 or p.x > size.x or p.y < 0:
		return -1
	var i := int(p.y / step)
	if i < 0 or i >= SLOTS:
		return -1
...
func _draw() -> void:
	...
	for i in SLOTS:
		var r := Rect2((size.x - slot_w) / 2.0, i * step + (step - slot_h) / 2.0, slot_w, slot_h)
...
func select(i: int) -> void:
	if i < 0 or i >= SLOTS:
		return
	group_selected.emit(i, world.select_units(groups[i]))


func focus(i: int) -> void:
	if i < 0 or i >= SLOTS or groups[i].is_empty():
		return
	world.select_units(groups[i])
	group_focused.emit(i)


func assign(i: int) -> void:
	if i < 0 or i >= SLOTS:
		return
	var list: Array = groups[i].duplicate() if world.additive_select else []
	for u in world.selection:
		if u.alive and not world.types[u.type].get("building", false) and not list.has(u):
			list.append(u)
	groups[i] = list
	group_assigned.emit(i, list.size())

...
def pal_name_for(tileset: str) -> str:
    return PALETTES[tileset]
...
def main() -> int:
    ...
    ap.add_argument("--recipe", type=Path, default=None,
                    help="zusätzlich das Gerät-Rezept schreiben (Verzeichnis; <tileset>.recipe.json)")
    ap.add_argument("--recipe-only", action="store_true",
                    help="nur das Rezept schreiben, keinen Atlas packen (braucht trotzdem den Inhalt)")
    a = ap.parse_args()
    ...
    smudge_ext = ext if tileset in ("snow", "desert") else "tem"
    for stem in ["sc1", "sc2", "sc3", "sc4", "sc5", "sc6", "cr1", "cr2", "cr3", "cr4", "cr5", "cr6"]:
        add_sprite(stem, f"{stem}.{smudge_ext}")
    ...
    if a.recipe is not None:
        ...
        lines = [
            "format 1",
            f"tileset {tileset}",
            f"palette {pal_name_for(tileset)}",
            "max_width 4096",
            "bits_first " + " ".join(sorted(Content.BITS_FIRST)),
        ]
        lines += [f"sprite {k} {sprite_files[k]}" for k in sprites]
        lines.append("meta " + json.dumps(meta, separators=(",", ":"))[1:-1])
        (a.recipe / f"{tileset}.recipe").write_text("\n".join(lines) + "\n")
        print(f"{tileset}.recipe: {len(sprites)} Sprites → {a.recipe}")
        if a.recipe_only:
            return 0

...
from rafmt.miniyaml import Node, Rules, parse, parse_file
...
SIDE_OF_SUBFACTION = {"england": "allies", "france": "allies", "germany": "allies",
                      "russia": "soviet", "ukraine": "soviet"}


SIDE_GRANT_PREFIXES = ("vehicles.", "infantry.")
SIDE_GRANT_EXCLUDE = {"infantry.england"}


def with_side_factions(prerequisite: str, factions: list[str]) -> list[str]:

    if not prerequisite.startswith(SIDE_GRANT_PREFIXES) or prerequisite in SIDE_GRANT_EXCLUDE:
        return factions
    out = list(factions)
    for f in factions:
        side = SIDE_OF_SUBFACTION.get(f)
...
VOLKOV_VOICES = """VolkovVoice:
\tVoices:
\t\tSelect: volksel1,volksel2,volksel3,volksel4
\t\tMove: volkmov1,volkmov2,volkmov3
\t\tAction: volkatk1,volkatk2,volkatk3
\t\tDie: dedman1,dedman2,dedman3,dedman4,dedman5,dedman7,dedman8
\t\tBurned: dedman10
\t\tZapped: dedman6
\t\tBuild: volkbld1
\t\tKill: volkkil1,volkkil2
\t\tDemolish: volkdem1,volkdem2
"""
        ...
        for actor in nodes:
            ...
            for sq in actor.children:
                ...
                seqs[sq.key] = {
                    ...
                    "zoffset": wdist(str(d.get("ZOffset", "0"))),


                    "flipx": str(d.get("FlipX", "false")).lower() == "true",
...
BALANCE_WEAPON_PERCENT_FIELDS = {"DamagePercent"}


_BALANCE_RULES: dict[str, bool] | None = None


def balance_rules(path: Path = BALANCE_FILE) -> dict[str, bool]:

    global _BALANCE_RULES
    if _BALANCE_RULES is None:
        _BALANCE_RULES = {}
        if path.exists():
...
def apply_actor_balance(act: dict, path: Path = BALANCE_FILE) -> bool:
    ...
    if not rules.get("c4_vehicles"):
        return False
    if act.get("building") or act.get("aircraft") or act.get("demolishable"):
        return False
    if "Vehicle" not in (act.get("target_types") or []):
        return False
    act["demolishable"] = True
    return True
    ...
    for top in parse_file(path):
        ...
        for wnode in top.children:
            ...
            for f in wnode.children:
                if f.key in BALANCE_WEAPON_PERCENT_FIELDS:
                    pct = int(f.value)
                    if "damage" in weapon:
                        weapon["damage"] = weapon["damage"] * pct // 100
                    for wh in weapon.get("warheads", []):
                        if "damage" in wh:
                            wh["damage"] = wh["damage"] * pct // 100
                    n += 1
                    continue
...
def extract_weapon(name: str, w: Node, seqs: dict[str, dict[str, dict]]) -> dict:
    ...
    for c in w.children:
        if not c.key.startswith("Warhead"):
            continue
        wd = c.as_dict()
        if c.value in ("SpreadDamage", "TargetDamage", "HealthPercentageDamage"):
            ...
            out["warheads"].append({
                ...
                "impact_sound_chance": int(wd.get("ImpactSoundChance", 100)),
                "impact_actors": str(wd.get("ImpactActors", "true")).lower() != "false",
                "delay": int(wd.get("Delay", 0)),

                "valid_targets": csv(str(wd.get("ValidTargets", "Ground, Water"))),
                "invalid_targets": csv(str(wd.get("InvalidTargets", ""))),
            })
        elif c.value == "LeaveSmudge":


            size = ints(str(wd.get("Size", "0,0")))
            out["warheads"].append({
                "type": "LeaveSmudge",
                "smudge_types": csv(str(wd.get("SmudgeType", "Crater,Scorch"))),
                "size": size[0] if size else 0,
                "size_inner": size[1] if len(size) > 1 else 0,
                "chance": int(wd.get("Chance", 100)),
                "valid_targets": csv(str(wd.get("ValidTargets", ""))),
                "invalid_targets": csv(str(wd.get("InvalidTargets", ""))),
            })
...
def extract_actor(name: str, r: Node, seqs: dict, fluent: dict) -> dict:
    ...
    if b is not None:
        ...
        out["buildable"] = {
            ...
            "at_production": str(b.get("BuildAtProductionType", "") or ""),
        }
    if has("Valued"):
    ...
    if has("Aircraft"):
        ...
        out["aircraft"] = {
            ...
            "repulsable": (ac.get("Repulsable", "true") or "true").lower() == "true",
            "repulsion_speed": int(ac.get("RepulsionSpeed", "-1") or -1),
            "ideal_separation": wdist(str(ac.get("IdealSeparation", "1706"))),
        }
    for c in r.children:
    ...
    if has("FallsToEarth"):
        ...
        spin = g("FallsToEarth", "MaximumSpinSpeed", None)
        out["falls_to_earth"] = {
            "moves": (g("FallsToEarth", "Moves", "false") or "false").lower() == "true",
            "velocity": wdist(str(g("FallsToEarth", "Velocity", "43") or "43")),
            "explosion": str(g("FallsToEarth", "Explosion", "UnitExplode") or "").lower(),
            "max_spin": -1 if spin in (None, "") else int(spin),
        }
    ...
    turrets = []
    for c in r.children:
        if c.key != "Turreted" and not c.key.startswith("Turreted@"):
            continue
        cd = c.as_dict()
        turrets.append({
            "name": (cd.get("Turret", "primary") or "primary").lower(),
            "turn_speed": int(cd.get("TurnSpeed", "512") or 512),
            "initial_facing": int(cd.get("InitialFacing", "0") or 0),
            "realign_delay": int(cd.get("RealignDelay", "40") or 40),
            "offset": ints(cd.get("Offset", "0,0,0") or "0,0,0"),
        })
    if turrets:
        out["turret"] = {k: v for k, v in turrets[0].items() if k in ("turn_speed", "initial_facing", "realign_delay")}
        out["turrets"] = turrets
    arms = []
    for c in r.children:
        if c.key == "Armament" or c.key.startswith("Armament@"):
            ...
            arms.append({
                ...
                "muzzle_sequence": ad.get("MuzzleSequence", ""),
                "relationships": csv(ad.get("TargetRelationships", "Enemy")),
            })
    if arms:
        out["armaments"] = arms

    if has("WithMuzzleOverlay"):
        out["with_muzzle_overlay"] = True
        out["muzzle_ignore_offset"] = str(g("WithMuzzleOverlay", "IgnoreOffset", "false")).lower() == "true"
    ...
    if has("AutoTarget"):
        ...
        if g("AutoTarget", "InitialStance", ""):
            out["initial_stance"] = g("AutoTarget", "InitialStance", "")
        if g("AutoTarget", "InitialStanceAI", ""):
            out["initial_stance_ai"] = g("AutoTarget", "InitialStanceAI", "")
    ...
    if bld is not None:
        out["building"] = {
            ...
            "requires_base_provider": bld.get("RequiresBaseProvider", "False").strip().lower() == "true",
        }
    ...
    for c in r.children:
        if c.key == "ProvidesPrerequisite" or c.key.startswith("ProvidesPrerequisite@"):
            pd = c.as_dict()
            prereq = pd.get("Prerequisite", name)
            provides.append({"prerequisite": prereq,
                             "factions": with_side_factions(prereq, csv(pd.get("Factions", ""))),
    ...
    for c in r.children:
        if c.key == "FireWarheadsOnDeath":
            out["death_weapon"] = c.get("Weapon", "").lower()
        elif c.key.startswith("FireWarheadsOnDeath@") and "death_weapon" not in out:
            out["death_weapon"] = c.get("Weapon", "").lower()
    ...
    if has("Husk"):
        ...
        out["husk"] = True
        out["husk_terrain"] = csv(g("Husk", "AllowedTerrain", ""))


        out["husk_locomotor"] = str(g("Husk", "Locomotor", "tracked")).lower()


        for ov in overlays:
            if not ov.get("image"):
                continue
            sq = seqs.get(ov["image"], {}).get(str(ov["sequence"]))
            if sq is not None:
                out["husk_overlay"] = dict(sq)
                break
    if has("SpawnActorOnDeath"):


        out["spawn_on_death"] = {
            "actor": str(g("SpawnActorOnDeath", "Actor", "")).lower(),
            "probability": int(g("SpawnActorOnDeath", "Probability", "100") or 100),
        }
    if has("Cloak"):

        cloak_pause = g("Cloak", "PauseOnCondition", "")


        pause_critical = False
        if cloak_pause:
            for c in r.children:
                if c.key != "GrantConditionOnDamageState" and not c.key.startswith("GrantConditionOnDamageState@"):
                    continue
                cd = c.as_dict()
                if cd.get("Condition", "") == cloak_pause and "Critical" in csv(cd.get("ValidDamageStates", "")):
                    pause_critical = True
        out["cloak"] = {
            ...
            "sound": Path(str(g("Cloak", "CloakSound", "") or g("Cloak", "UncloakSound", ""))).stem.lower(),
        }
    if has("DetectCloaked"):
        ...
        out["detect_cloaked"] = {
            ...
            "render_circle": has("RenderDetectionCircle"),
        }
    if has("Mine"):
    ...
    if has("MadTank"):
        ...
        out["mad_tank"] = {
            "thump_sequence": str(g("MadTank", "ThumpSequence", "piston") or "piston"),
            "thump_interval": int(g("MadTank", "ThumpInterval", "8") or 8),
            "thump_weapon": str(g("MadTank", "ThumpDamageWeapon", "MADTankThump") or "MADTankThump").lower(),
            "charge_delay": int(g("MadTank", "ChargeDelay", "96") or 96),
            "charge_sound": Path(str(g("MadTank", "ChargeSound", "madchrg2.aud") or "madchrg2.aud")).stem.lower(),
            "detonation_delay": int(g("MadTank", "DetonationDelay", "42") or 42),
            "detonation_sound": Path(str(g("MadTank", "DetonationSound", "madexplo.aud") or "madexplo.aud")).stem.lower(),
            "detonation_weapon": str(g("MadTank", "DetonationWeapon", "MADTankDetonate") or "MADTankDetonate").lower(),
            "driver": str(g("MadTank", "DriverActor", "e1") or "e1").lower(),
        }
    if has("GrantConditionOnDeploy") and has("KillsSelf"):


        cond = str(g("GrantConditionOnDeploy", "DeployedCondition", "") or "")
        req = str(g("KillsSelf", "RequiresCondition", "") or "")
        if cond and cond in req:
            out["detonate_on_deploy"] = True
    if has("PortableChrono"):


        out["portable_chrono"] = {
            "charge_delay": int(g("PortableChrono", "ChargeDelay", "500") or 500),
            "max_distance": int(g("PortableChrono", "MaxDistance", "12") or 12),
            "sound": Path(str(g("PortableChrono", "ChronoshiftSound", "chrotnk1.aud") or "chrotnk1.aud")).stem.lower(),
            "kill_cargo": str(g("PortableChrono", "KillCargo", "True") or "True") == "True",
        }
    ...
    if has("RepairsBridges"):
        out["repairs_bridges"] = True
    ...
    if has("Infiltrates"):
        ...
        rel = str(g("Infiltrates", "ValidRelationships", "Enemy, Neutral") or "Enemy, Neutral")
        rels = [x.strip().lower() for x in rel.split(",")]
        types_ = csv(g("Infiltrates", "Types", "") or "")
        if "ally" in rels:
            out["infiltrates_ally"] = types_
        if "enemy" in rels or "neutral" in rels:
            out["infiltrates"] = types_
    ...
    if has("InfiltrateForCash"):
        ...
        out["infil_cash_max"] = int(c.get("Maximum", "2147483647") or 2147483647)
    if has("InfiltrateForExploration"):
        out["infil_explore"] = csv(g("InfiltrateForExploration", "Types", "") or "")


    if has("TransformOnCapture"):
        c = r.child("TransformOnCapture")
        out["capture_into"] = str(c.get("IntoActor", "") or "").lower()
        out["capture_health"] = int(c.get("ForceHealthPercentage", "0") or 0)
    if has("InfiltrateForTransform"):
        c = r.child("InfiltrateForTransform")
        out["infil_transform"] = csv(c.get("Types", "") or "")
        if not out.get("capture_into"):
            out["capture_into"] = str(c.get("IntoActor", "") or "").lower()
            out["capture_health"] = int(c.get("ForceHealthPercentage", "0") or 0)
    ...
    if has("InfiltrateForSupportPowerReset"):


        out["infil_reset"] = csv(g("InfiltrateForSupportPowerReset", "Types", "") or "")
    if has("Cargo"):
        ...
        out["cargo_delays"] = [int(g("Cargo", "BeforeUnloadDelay", "8") or 8),
                               ...
                               int(g("Cargo", "AfterLoadDelay", "8") or 8)]
        out["cargo_eject_on_death"] = (g("Cargo", "EjectOnDeath", "false") or "false").lower() == "true"
    if has("WithLandingCraftAnimation"):


        out["landing_craft"] = csv(g("WithLandingCraftAnimation", "OpenTerrainTypes", "Clear") or "Clear")
    if has("Passenger"):
    ...
    gap_reveal = 0
    for c in r.children:
        if c.key != "RevealsShroud" and not c.key.startswith("RevealsShroud@"):
            continue
        if (c.get("RequiresCondition", "") or "").strip() == "disabled":
            continue
        if str(c.get("RevealGeneratedShroud", "True") or "True").lower() == "false":
            continue
        gap_reveal = max(gap_reveal, wdist(c.get("Range", "0") or "0"))
    if gap_reveal:
        out["reveals_gap"] = gap_reveal


    if has("CreatesShroud"):
        out["creates_shroud"] = wdist(g("CreatesShroud", "Range", "0") or "0")


    for c in r.children:
        if c.key != "ProximityExternalCondition" and not c.key.startswith("ProximityExternalCondition@"):
            continue
        if str(c.get("Condition", "") or "").strip().lower() != "jammed":
            continue
        out["jammer_range"] = wdist(c.get("Range", "3c0") or "3c0")
...
def main() -> int:
    ...
    for name in rules.names():
        ...
        n_c4 += 1 if apply_actor_balance(act) else 0
        if name == "volk":


            act["sequences"] = dict(act["sequences"])
            act["sequences"]["icon"] = {"file": "volkicon.shp", "start": 0, "length": 1, "facings": 1,
                                         "tick": 40, "stride": 0, "offset": [0, 0], "zoffset": 0,
                                         "flipx": False, "classic": False,
    ...
    for vs in parse_file(mod / "audio" / "voices.yaml") + parse(VOLKOV_VOICES):
    ...
    music: dict = {"hidden": [], "volume": {}, "title": {}, "victory": "", "defeat": ""}
    for n in parse_file(mod / "audio" / "music.yaml"):
        ...
        if n.value:
            music["title"][n.key] = n.value.strip()
    ...
    print(f"{len(actors)} Actors ({n_sup} unterstützt, {n_c4} Fahrzeuge sprengbar aus balance.yaml), "
          f"{len(wdict)} Waffen ({n_balance} Balance-Feld"

...
constexpr WDist WAIT_DISTANCE_FROM_RESUPPLY_BASE = 3072;
...
void World::air_repulse(size_t i) {
    Actor& a = actors_[i];
    Air& air = airs_[i];
    Mobile& m = mobiles_[i];
    const UnitType& t = types_[a.type];
    ...
    if (!t.repulsable || t.ideal_separation <= 0) return;


    if (air.base >= 0) {
        const int b = index_of(air.base);
        if (b >= 0 && actors_[size_t(b)].alive &&
            length(actors_[size_t(b)].pos - a.pos) < WAIT_DISTANCE_FROM_RESUPPLY_BASE)
            return;
    }


    if (air.state != Air::CRUISING || air.alt != t.cruise_altitude) return;

    const int64_t sep_sq = int64_t(t.ideal_separation) * t.ideal_separation;
    int64_t fx = 0, fy = 0;


    for (size_t j = 0; j < actors_.size(); ++j) {
        if (j == i) continue;
        const Actor& o = actors_[j];
        if (!o.alive || !in_world(j)) continue;
        const UnitType& ot = types_[o.type];
        if (!ot.aircraft || !ot.repulsable || ot.cruise_altitude != t.cruise_altitude) continue;

        if (airs_[j].alt < air.alt) continue;
        const WVec d{a.pos.x - o.pos.x, a.pos.y - o.pos.y};
        const int64_t dsq = length_sq(d);
        if (dsq > sep_sq) continue;
        if (dsq < 1) {

            const WVec r = direction_of(WAngle(rand() % 1023));
            fx += r.x;
            fy += r.y;
            continue;
        }

        fx += int64_t(d.x) * 1024 * 8 / dsq;
        fy += int64_t(d.y) * 1024 * 8 / dsq;
    }
    if (fx == 0 && fy == 0) return;


    const auto clamp32 = [](int64_t v) {
        constexpr int64_t LIM = 1 << 24;
        return static_cast<WDist>(v > LIM ? LIM : (v < -LIM ? -LIM : v));
    };
    const WVec force{clamp32(fx), clamp32(fy)};
    if (!t.can_hover) {


        const WVec dir = direction_of(a.facing);
        const int64_t len = int64_t(length(dir)) * length(force);
        if (len == 0) return;
        const int64_t dot = (int64_t(dir.x) * force.x + int64_t(dir.y) * force.y) / len;
        if (dot < 0) return;
    }


    const int32_t rspeed = t.repulsion_speed >= 0 ? t.repulsion_speed : std::max(1, t.speed);
    const WVec step_dir = direction_of(angle_of(force));
    a.pos.x += static_cast<WDist>(int64_t(step_dir.x) * rspeed / 1024);
    a.pos.y += static_cast<WDist>(int64_t(step_dir.y) * rspeed / 1024);

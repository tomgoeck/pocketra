...
void World::spawn_effect(WVec pos, int32_t seq, WAngle facing, WDist alt) {
    if (seq < 0) return;
    effects_.push_back({pos, seq, 0, 0, facing, alt});
}
...
int World::nearest_enemy_in_range(size_t i, WDist range, bool auto_scan) const {
    int best = -1;
    ...
    const uint32_t stance = combats_[i].stance >= STANCE_ATTACK_ANYTHING ? (base_mask | TT_STRUCTURE) : base_mask;
...
void World::order_attack(const int32_t* ids, size_t n, int32_t target_id) {
    const int ti = index_of(target_id);
    if (ti < 0 || !actors_[ti].alive) return;
    for (size_t k = 0; k < n; ++k) {
        const int i = index_of(ids[k]);
...
void World::fire(size_t i, size_t target, int32_t weapon_id) {
...
void World::fire_at(size_t i, WVec aim_at, WDist target_alt, int32_t weapon_override, int32_t target_actor_id) {
    ...
    const int arm = (weapon_id == ut.weapon) ? 0 : 1;
    ...
    if (w.speed <= 0) {
        impact(aim, weapon_id, a.id, a.owner, target_alt);
    } else {
        Projectile p;
        p.pos = a.pos;
        p.alt = airs_[i].alt;
...
void World::step_combat(size_t i) {
    ...
    if (ti < 0 && map_.in_bounds(c.attack_cell)) {
        ...
        bool ok;
        if (t.turreted) {
            c.turret = turn_towards(c.turret, want_cell, t.turret_turn);
            c.realign = 0;
            ok = abs_angle_diff(c.turret, want_cell) <= AIM_TOLERANCE;
        } else if (t.building) {
            ok = true;
        } else {
        ...
        if (ok && c.reload == 0) fire_at(i, aim);
        return;
    }
    ...
    if (ti < 0 && c.guard >= 0) {
        ...
        if (gi < 0 || !actors_[gi].alive) {
            ...
            if (t.turreted && !m.moving) {
                ...
                else c.turret = turn_towards(c.turret, a.facing, t.turret_turn);
            }
            return;
        }
    }
    ...
    if (t.turreted) {
        ...
        c.turret = turn_towards(c.turret, want, t.turret_turn);
        c.realign = 0;
        ready = abs_angle_diff(c.turret, want) <= AIM_TOLERANCE;
    } else if (t.building) {
        ready = true;
    } else {
...
void World::step_projectiles() {
    for (Projectile& p : projectiles_) {
        ...
        if (len <= w.speed) {
            p.pos = p.target;
            p.alt = p.target_alt;
            impact(p.pos, p.weapon, p.source, p.owner, p.alt);
            p.alive = false;
        } else {
...
void World::step_missile(Projectile& p) {
    ...
    if (close_enough || out_of_fuel) {
        if (p.ticks_alive > w.missile_arm) impact(p.pos, p.weapon, p.source, p.owner, p.alt);
        p.alive = false;
    }
}
...
void World::apply_damage_warhead(WVec pos, int32_t attacker_id, int32_t attacker_owner, WDist spread,
                                  ...
                                  int32_t relation, int32_t damage_type, bool trigger_prone, int32_t prone_damage) {
    ...
    for (size_t k = 0; k < actors_.size(); ++k) {
        ...
        if (ally_weapon ? !allied(attacker_owner, v.owner) : (allied(attacker_owner, v.owner) && !neutral_hit)) continue;
        ...
        int64_t dmg = int64_t(damage) * fo / 100 * versus[vt.armor] / 100;
        ...
        if (vc.target < 0 && vc.stance > STANCE_HOLD_FIRE && vt.weapon >= 0 && weapons_[vt.weapon].relation == REL_ENEMY) {
...
void World::impact(WVec pos, int32_t weapon_id, int32_t attacker_id, int32_t attacker_owner, WDist alt,
    ...
    if (w.cluster_weapon >= 0 && depth == 0) {
        ...
        for (int32_t k = 0; k < w.cluster_count; ++k) {
            ...
            if (map_.in_bounds(t)) impact(cell_center(t), w.cluster_weapon, attacker_id, attacker_owner, alt, depth + 1);
        }
    }
    ...
    if (w.delay <= 0) {
        ...
        spawn_effect(pos, w.impact_effect_count > 1
                              ? w.impact_effects[rand() % uint32_t(w.impact_effect_count)]
                              : w.impact_effect, 0, alt);
        play_sound(w.impact_sound, pos);
        apply_damage_warhead(pos, attacker_id, attacker_owner, w.spread, w.damage, w.falloff, w.falloff_steps,
                              w.versus, w.valid_targets, w.invalid_targets, w.relation, w.damage_type,
                              w.trigger_prone, w.prone_damage);
    } else {
    ...
    for (int32_t i = 0; i < w.extra_warhead_count; ++i) {
        ...
        if (e.delay <= 0) {
            apply_damage_warhead(pos, attacker_id, attacker_owner, e.spread, e.damage, e.falloff,
                                  ...
                                  w.relation, w.damage_type, e.trigger_prone, e.prone_damage);
        } else {
...
void World::step_pending_impacts() {
    ...
    for (size_t i = 0; i < pending_impacts_.size(); ++i) {
        ...
        const Weapon& w = weapons_[p.weapon];
        if (p.kind == PendingImpact::DAMAGE && p.index < 0) {


            spawn_effect(p.pos, w.impact_effect_count > 1
                                     ? w.impact_effects[rand() % uint32_t(w.impact_effect_count)]
                                     : w.impact_effect, 0, p.alt);
            play_sound(w.impact_sound, p.pos);
            apply_damage_warhead(p.pos, p.attacker_id, p.attacker_owner, w.spread, w.damage, w.falloff,
                                  ...
                                  w.relation, w.damage_type, w.trigger_prone, w.prone_damage);
...
void World::kill(size_t i, int32_t damage_type, int32_t attacker_id) {
    ...
    spawn_effect(a.pos, fx, a.facing, death_alt);
...
void World::render_sprites(int32_t alpha, std::vector<RenderSprite>& out) const {
    ...
    for (const Effect& e : effects_) out.push_back({e.pos.x, e.pos.y, e.alt, -1, e.seq, e.frame, e.facing});
}

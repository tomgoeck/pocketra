...
void World::update_visibility(int32_t owner) {
    ...
    for (uint8_t& v : vis) if (v == 2) v = 1;
    auto reveal_circle = [&](CPos c, int32_t range) {
        ...
        for (int dy = -r; dy <= r; ++dy) {
            for (int dx = -r; dx <= r; ++dx) {
                ...
                if (map_.in_bounds(p)) vis[map_.index(p)] = 2;
            }
        }
    };
    for (size_t k = 0; k + 2 < reveals_.size(); k += 3) {
        if (!allied(owner, reveals_[k].x)) continue;
        reveal_circle(reveals_[k + 1], reveals_[k + 2].x * CELL);
    }
    for (size_t i = 0; i < actors_.size(); ++i) {
        const Actor& a = actors_[i];
        if (!in_world(i) || !allied(owner, a.owner)) continue;
        const UnitType& t = types_[a.type];
        if (t.reveal_range <= 0) continue;
        if (t.building) {
            for (int y = 0; y < t.foot_h; ++y) {
                for (int x = 0; x < t.foot_w; ++x) {
                    if (!t.footprint[size_t(y * t.foot_w + x)]) continue;
                    reveal_circle(CPos{a.origin.x + x, a.origin.y + y}, t.reveal_range);
                }
            }
        } else {
            reveal_circle(mobiles_[i].cell, t.reveal_range);
        }
    }
    ...
    for (size_t i = 0; i < actors_.size(); ++i) {
        ...
        if ((seen_mask_[i] >> owner) & 1u) continue;
        const UnitType& t = types_[a.type];
        for (int y = 0; y < t.foot_h && !((seen_mask_[i] >> owner) & 1u); ++y) {
            for (int x = 0; x < t.foot_w; ++x) {
                const CPos c{a.origin.x + x, a.origin.y + y};
                if (map_.in_bounds(c) && vis[map_.index(c)] == 2) {
                    seen_mask_[i] |= 1u << owner;
                    break;
                }
            }
        }
...
void World::step_order_queue() {
    for (size_t i = 0; i < actors_.size(); ++i) {
        ...
        case QueuedOrder::ATTACK: order_attack(&id, 1, o.target); break;
...
int World::push_actor(const Actor& a, const Mobile& m) {
    ...
    if (a.owner >= 0 && a.owner < MAX_PLAYERS && players_[a.owner].bot.enabled) cb.stance = STANCE_ATTACK_ANYTHING;
...
transit:
    ...
    if (t.turreted && a.facing != facing_before)
        combats_[i].turret = wrap_angle(combats_[i].turret + angle_diff(facing_before, a.facing));
    if (!t.infantry) {
...
void World::step() {
    ...
    step_effects();
    step_zaps();
    if (tick_ % 5 == 0) update_visibility(0);
...
void World::render(int32_t alpha, RenderActor* out) const {
    for (size_t i = 0; i < actors_.size(); ++i) {
        const Actor& a = actors_[i];
        const Mobile& m = mobiles_[i];
        const Combat& c = combats_[i];
        ...
        r.visible = actor_visible_to(0, i) ? 1 : 0;

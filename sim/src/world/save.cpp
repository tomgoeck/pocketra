...
template <class V> void visit_fields(V& v, Harvest& h) {
    ...
    v(h.stall_ticks); v(h.dock_fails); v(h.avoid_proc); v(h.avoid_proc_ticks);
}
...
template <class V> void visit_fields(V& v, BotParams& p) {
    ...
    v(p.harv_stall_ticks); v(p.harvesters_extra);

    v(p.straggler_min_cells); v(p.straggler_interval); v(p.gather_ready_percent);
    v(p.gather_repicks_max); v(p.no_flee_ticks); v(p.counter_push_on);


    v(p.defense_aa_max); v(p.defense_army_percent); v(p.defense_growth_tick);


    v(p.air_soft_targets);
    v(p.naval_escort); v(p.naval_escort_per_siege); v(p.naval_escort_cells);
    v(p.naval_retreat_cells); v(p.naval_hunt_cloaked); v(p.naval_memory_ticks);
    v(p.naval_siege_range_cells);


    v(p.queue_budget); v(p.naval_unit_limit); v(p.naval_escort_per_bombard);
    v(p.continuous_production); v(p.wave_value);

    v(p.opening_plan); v(p.opening_refineries); v(p.opening_factory_after);
    v(p.opening_production_free); v(p.opening_defense_per); v(p.opening_attack_ticks);
}
...
template <class V> void visit_fields(V& v, BotSquad& s) {
    ...
    v(s.stat_last_tick); v(s.march_start); v(s.contact); v(s.peak_size);
    v(s.gather_picks); v(s.straggler_tick); v(s.retired);
}
...
template <class V> void visit_fields(V& v, BotState& b) {
    ...
    v(b.stat_harv_worst); v(b.stat_harv_peak); v(b.stat_harv_samples);
    v(b.stat_harv_state); v(b.stat_harv_ore);


    v(b.stat_squad_gather_ticks); v(b.stat_squad_march_ticks);
    v(b.stat_squad_contacts); v(b.stat_squad_contact_ticks); v(b.stat_squads_no_contact);
    v(b.stat_gather_repicks); v(b.stat_straggler_stops); v(b.stat_attacks_heavy_loss);


    v(b.sea_seen_ids); v(b.sea_seen_cells); v(b.sea_seen_ticks);
    v(b.stat_air_attacks); v(b.stat_air_lost); v(b.stat_ships_lost); v(b.stat_ships_killed);


    v(b.naval_demand); v(b.naval_enemy_value); v(b.naval_shore_targets); v(b.naval_scan_ticks);
    v(b.counter_naval); v(b.stat_built_q);


    v(b.stat_t_refinery); v(b.stat_t_barracks); v(b.stat_t_factory); v(b.stat_t_radar);
    v(b.last_attack_tick);
}

...
constexpr int32_t MAX_DOCK_FAILS = 3;
constexpr int32_t DOCK_AVOID_TICKS = 500;
...
bool World::claim_taken(int cell_idx, int32_t self) const {
    if (cell_idx < 0 || size_t(cell_idx) >= claims_.size()) return false;
    const int32_t cl = claims_[size_t(cell_idx)];
    if (cl < 0 || cl == self) return false;
    if (size_t(cl) >= actors_.size()) return false;
    if (!in_world(size_t(cl)) || !types_[actors_[size_t(cl)].type].harvester) return false;
    return harvests_[size_t(cl)].claim == cell_idx;
}
...
int World::choose_refinery(size_t i) const {
    ...
    for (size_t k = 0; k < actors_.size(); ++k) {
        ...
        if (me.avoid_proc_ticks > 0 && o.id == me.avoid_proc) continue;
...
void World::step_harvester(size_t i) {
    ...
    if (h.stall_ticks < INT32_MAX) ++h.stall_ticks;

    if (h.avoid_proc_ticks > 0 && --h.avoid_proc_ticks <= 0) h.avoid_proc = -1;
    ...
    case Harvest::TO_DOCK: {
        ...
        if (m.cell == dock && !m.in_transit) {
            ...
            if (++h.dock_fails > MAX_DOCK_FAILS) {
                h.dock_fails = 0;
                h.dock_held = false;
                h.linked_proc = -1;
                h.avoid_proc = h.proc;
                h.avoid_proc_ticks = DOCK_AVOID_TICKS;
                h.proc = -1;
                h.timer = 0;
                break;
            }

...
void World::buildable(int32_t owner, int32_t kind, std::vector<int32_t>& out) const {
    ...
    for (size_t t = 0; t < types_.size(); ++t) {
        ...
        if (!prerequisites_met(owner, static_cast<int32_t>(t))) continue;


        if (ut.aircraft && free_landing_pads(owner, static_cast<int32_t>(t)) <= 0) continue;
...
void World::step_production() {
    ...
    for (int32_t owner = 0; owner < MAX_PLAYERS; ++owner) {
        ...
        const bool low_power = drained[owner] > provided[owner] && drained[owner] > 0;
        if (!low_power) {
            power_notified_[owner] = NOTIFY_NEVER;
        } else if (power_notified_[owner] == NOTIFY_NEVER || tick_ - power_notified_[owner] > 10 * TICKS_PER_SECOND) {
        ...
        for (int32_t kind = 0; kind < NUM_QUEUES; ++kind) {
            ...
            int producer = find_producer(owner, kind);
            ...
            if (idx >= 0) {
                ...
                if (air) {
                    const std::vector<int32_t>& rb = pads_of(types_[item.type]);
...
bool World::set_rally(int32_t id, CPos cell) {
    ...
    const UnitType& bt = types_[actors_[i].type];
    if (bt.produces == 0 || !map_.in_bounds(cell)) return false;


    if ((bt.produces & (1u << QUEUE_SHIP)) != 0) {
        const int32_t mc = move_class_of(LOCO_NAVAL);
        if (!map_.passable(cell, mc)) {
            CPos best{-1, -1};
            for (int rr = 1; rr < 12 && best.x < 0; ++rr) {
                for (int dy = -rr; dy <= rr && best.x < 0; ++dy) {
                    for (int dx = -rr; dx <= rr; ++dx) {
                        if (dx != -rr && dx != rr && dy != -rr && dy != rr) continue;
                        const CPos c{cell.x + dx, cell.y + dy};
                        if (map_.in_bounds(c) && map_.passable(c, mc)) { best = c; break; }
                    }
                }
            }
...
CPos World::enter_origin(size_t t) const {
    return types_[actors_[t].type].building ? actors_[t].origin : mobiles_[t].cell;
}
...
int32_t World::enter_kind_for(size_t i, size_t t) const {
    ...
    if (s.repairs_bridges && (target_mask(t) & uint32_t(TT_BRIDGE)) != 0 && actors_[t].hp < b.hp)
        return ENTER_REPAIR_BRIDGE;


    if (ally) {
        if (s.infiltrates_ally != 0 && (s.infiltrates_ally & target_mask(t)) != 0 &&
            (b.infil_transform & s.infiltrates_ally) != 0)
            return ENTER_INFILTRATE;
...
bool World::order_disguise(int32_t id, int32_t target_id) {
    ...
    const uint32_t tm = target_mask(size_t(t));
    if (tm != 0 && (tm & uint32_t(TT_DISGUISE)) == 0) return false;
...
void World::enter_effect(size_t i, size_t t) {
    ...
    case ENTER_CAPTURE: {
        ...
        const int32_t new_owner = a.owner;
        set_owner(t, new_owner);


        if (types_[actors_[t].type].capture_into >= 0) {
            notify(new_owner, NOTIFY_UNIT_STOLEN);
            transform_captured(t, new_owner);
            dispose(i);
            break;
        }


        notify(a.owner, structure ? NOTIFY_BUILDING_CAPTURED : NOTIFY_UNIT_STOLEN);
        if (!structure) notify(old_owner, NOTIFY_UNIT_LOST);
        dispose(i);
        break;
    }
    case ENTER_REPAIR:
...
void World::transform_captured(size_t t, int32_t owner) {
    const int32_t into = types_[actors_[t].type].capture_into;
    if (into < 0 || size_t(into) >= types_.size()) return;
    const int32_t pct = types_[actors_[t].type].capture_health_percent;
    const int32_t hp_pct = pct > 0 ? pct : 100;
    const bool building = types_[actors_[t].type].building;
    const CPos cell = building ? actors_[t].origin : mobiles_[t].cell;
    const WAngle facing = actors_[t].facing;
    dispose(t);
    if (types_[size_t(into)].building) spawn_building(into, owner, cell, hp_pct);
    else spawn(into, owner, cell, facing, hp_pct);
}
...
void World::infiltrate(size_t i, size_t t) {
    ...
    const uint32_t types = st.infiltrates | st.infiltrates_ally;


    if ((bt.infil_transform & types) != 0 && bt.capture_into >= 0) {
        transform_captured(t, actors_[i].owner);
        return;
    }


    if ((bt.infil_cash & types) != 0) {
        const int64_t take = std::min<int64_t>(bt.infil_cash_max,
                                               credits(victim) * bt.infil_cash_percent / 100);
    ...
    if ((bt.infil_explore & types) != 0) {
        if (spy_owner >= 0 && spy_owner < MAX_PLAYERS && vis_[spy_owner].size() == size_t(map_.cells()))
            explore_all(spy_owner);
        if (victim >= 0 && victim < MAX_PLAYERS) {
            reset_exploration(victim);
    ...
    if ((bt.infil_reset & types) != 0 && bt.support_power >= 0 && bt.support_power < SP_COUNT
            && victim >= 0 && victim < MAX_PLAYERS) {
        SupportPowerState& sp = players_[victim].powers[bt.support_power];


        if (!sp.used) { sp.charge = 0; sp.ready = false; }
    }
...
bool World::leave_building(size_t i, size_t t) {
    ...
    const int32_t lfw = bt.building ? bt.foot_w : 1;
    const int32_t lfh = bt.building ? bt.foot_h : 1;
    const CPos lorigin = enter_origin(t);
    ...
    for (int pass = 0; pass < 2 && !found; ++pass) {
        ...
        for (int y = -1; y <= lfh; ++y) {
            for (int x = -1; x <= lfw; ++x) {
                const bool inner = x >= 0 && x < lfw && y >= 0 && y < lfh;
                if (inner != (pass == 1)) continue;
                const CPos c{lorigin.x + x, lorigin.y + y};
...
void World::step_enter() {
    for (size_t i = 0; i < actors_.size(); ++i) {
        ...
        case ENTER_APPROACH: {
            ...
            const bool adjacent = c.x >= borigin.x - 1 && c.x <= borigin.x + bfw &&
                                  c.y >= borigin.y - 1 && c.y <= borigin.y + bfh;
            ...
            actors_[i].pos = cell_center(CPos{borigin.x + bfw / 2, borigin.y + bfh / 2});

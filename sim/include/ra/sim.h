...
struct Weapon {
    ...
    int32_t impact_effect = -1;
    int32_t impact_effects[4] = {-1, -1, -1, -1};
    int32_t impact_effect_count = 0;
    int32_t report_sound = -1;
    int32_t impact_sound = -1;
    WDist min_range = 0;
...
struct Combat {
    ...
    int32_t& reload_at(int k) { return k == 0 ? reload : reload2; }
    int32_t reload_at(int k) const { return k == 0 ? reload : reload2; }
    int32_t& burst_at(int k) { return k == 0 ? burst_left : burst2; }
    int32_t& since_at(int k) { return k == 0 ? since_shot : since_shot2; }
};


constexpr int NUM_ARMS = 2;
inline int32_t arm_weapon(const UnitType& t, int k) { return k == 0 ? t.weapon : t.weapon_secondary; }
...
public:
    ...
    bool cloaked(size_t i) const { return actors_[i].alive && types_[actors_[i].type].cloak && actors_[i].cloak_timer <= 0; }
    ...
    void order_attack(const int32_t* ids, size_t n, int32_t target_id);
    ...
    void destroy(int32_t id) { const int i = index_of(id); if (i >= 0) kill(size_t(i), DAMAGE_EXPLOSION); }
    ...
    void render(int32_t alpha_1024, RenderActor* out) const;
...
private:
    ...
    struct PendingImpact {
        enum Kind { DAMAGE = 0, DESTROY_RESOURCE = 1 };
        WVec pos;
        int32_t weapon = -1;
        int32_t kind = DAMAGE;
        int32_t index = 0;
    ...
    void step_effects();
    void step_zaps();
    int nearest_enemy_in_range(size_t i, WDist range, bool auto_scan) const;
    bool in_range(size_t i, size_t target, WDist range) const;


    void fire(size_t i, size_t target, int32_t weapon_id = -1);
    ...
    void spawn_effect(WVec pos, int32_t seq, WAngle facing = 0, WDist alt = 0);

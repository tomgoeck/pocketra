...
int web_shelf_pack(const std::vector<std::pair<int, int>>& items, std::vector<WebPlacement>& out_pos) {
    ...
    out_pos.assign(items.size(), WebPlacement());
    ...
    for (size_t i : order) {
        ...
        for (size_t p = 0; p < pages.size() && !placed; ++p) {
            std::vector<Row>& rows = pages[p];
            for (Row& row : rows) {
                if (h <= row.height && row.x + w <= WEB_PAGE) {
                    out_pos[i] = {(int)p, row.x, row.y};
                    row.x += w;
                    placed = true;
                    break;
                }
            ...
            if (next_y + h <= WEB_PAGE) {
                rows.push_back({h, w, next_y});
                out_pos[i] = {(int)p, 0, next_y};
                placed = true;
            }
        }
        if (!placed) {
            pages.push_back({Row{h, w, 0}});
            out_pos[i] = {(int)pages.size() - 1, 0, 0};
        }
    }
...
bool AtlasBuilder::build_one(const ContentSet& cs, const std::string& tileset, const std::string& out_dir,
    ...
    const bool make_web = web_pages_ && (width > WEB_PAGE || height > WEB_PAGE);
    std::vector<WebPlacement> wpos;
    std::vector<std::vector<uint8_t>> web_pages;
    std::string wjson;
    if (make_web) {
        for (const std::pair<int, int>& it : items) {

            if (it.first > 255 || it.second > 255) {
                error = tileset + ": Frame über 255 Pixel passt nicht in die Frametabelle";
                return false;
            }
        }
        const int page_count = web_shelf_pack(items, wpos);


        if (page_count > WEB_MAX_PAGES) {
            error = tileset + ": " + std::to_string(page_count) + " Browser-Seiten, der Shader hat " +
                    std::to_string(WEB_MAX_PAGES);
            return false;
        }
        web_pages.assign((size_t)page_count, std::vector<uint8_t>((size_t)WEB_PAGE * WEB_PAGE, 0));
        wjson.reserve(1 << 20);
        wjson += "{\"width\":";
        wjson += std::to_string(WEB_PAGE);
        wjson += ",\"height\":";
        wjson += std::to_string(WEB_PAGE);
        wjson += ",\"frames\":[";
    }
    ...
    for (size_t si = 0; si < sets.size(); ++si) {
        ...
        for (size_t fi = 0; fi < s.frames.size(); ++fi, ++k) {
            ...
            json += buf;
            if (make_web) {
                const WebPlacement& wp = wpos[k];
                std::vector<uint8_t>& page = web_pages[(size_t)wp.page];
                for (int row = 0; row < s.height; ++row) {
                    const size_t dst = (size_t)(wp.y + row) * WEB_PAGE + wp.x;
                    const size_t src = (size_t)row * s.width;
                    if (src + s.width <= f.size()) {
                        std::memcpy(page.data() + dst, f.data() + src, (size_t)s.width);
                    }
                }


                std::snprintf(buf, sizeof(buf),
                              "{\"sprite\":\"%s\",\"frame\":%zu,\"x\":%d,\"y\":%d,\"w\":%d,\"h\":%d,\"page\":%d}",
                              stems[si].c_str(), fi, wp.x, wp.y, s.width, s.height, wp.page);
                wjson += buf;
            }
        }
    }
    ...
    if (!make_web &&
        !write_whole_file(join_path(out_dir, name + ".r8"), atlas.data(), atlas.size())) {
    ...
    if (make_web) {
        ...
        wjson += ",\"pages\":";
        wjson += std::to_string((int)web_pages.size());
        wjson += '}';
        if (!write_whole_file(join_path(out_dir, name + "_web.json"), (const uint8_t*)wjson.data(),
                              wjson.size())) {
            error = "Atlasdaten (Browser) nicht schreibbar: " + out_dir;
            return false;
        }
        for (size_t p = 0; p < web_pages.size(); ++p) {
            if (progress) {
                progress("atlas", base + span * 0.9f, tileset + ": Seite " + std::to_string(p + 1));
            }
            const std::string page_path =
                    join_path(out_dir, name + "_web_" + std::to_string(p) + ".r8");
            if (!write_whole_file(page_path, web_pages[p].data(), web_pages[p].size())) {
                error = "Atlasseite nicht schreibbar: " + page_path;
                return false;
            }
        }


        for (size_t p = web_pages.size(); p < web_pages.size() + 8; ++p) {
            const std::string stale = join_path(out_dir, name + "_web_" + std::to_string(p) + ".r8");

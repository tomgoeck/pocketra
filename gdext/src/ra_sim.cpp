...
void RaSim::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("visibility_map", "owner"), &RaSim::visibility_map);
    ...
    ClassDB::bind_method(D_METHOD("destroy", "id"), &RaSim::destroy);
    ...
    ClassDB::bind_method(D_METHOD("order_attack", "ids", "target_id", "queued"), &RaSim::order_attack, DEFVAL(false));
    ...
    ClassDB::bind_method(D_METHOD("drain_notifications", "owner"), &RaSim::drain_notifications);
...
void RaSim::destroy(int id) { world_.destroy(id); }
...
PackedByteArray RaSim::visibility_map(int owner) const {
    const std::vector<uint8_t>& v = world_.visibility(owner);
    PackedByteArray out;
...
int RaSim::define_weapon(const Dictionary& def) {
    ...
    w.impact_effect = int(def.get("impact_effect", -1));
    const Array impacts = def.get("impact_effects", Array());
    for (int i = 0; i < impacts.size() && i < 4; ++i) w.impact_effects[w.impact_effect_count++] = int(impacts[i]);
    w.report_sound = int(def.get("report_sound", -1));
    w.impact_sound = int(def.get("impact_sound", -1));
...
void RaSim::order_attack(const PackedInt32Array& ids, int target_id, bool queued) {
    if (queued) {
        ra::QueuedOrder o;
    ...
    world_.order_attack(ids.ptr(), ids.size(), target_id);
}
...
PackedInt32Array RaSim::drain_notifications(int owner) {
    world_.drain_notifications(owner, int_scratch_);
    PackedInt32Array out;
...
PackedFloat32Array RaSim::render_buffer(float alpha) {
    ...
    auto sort_y = [&](int i) {
        return scratch_[i].y * WORLD_TO_PX + (scratch_[i].altitude > 0 ? 1.0e6f : 0.0f);
    };
    ...
    for (int i : order_) {
        ...
        if (sprites_[eff_sprite_type(scratch_[i])].turret_first >= 0) ++instances;
    ...
    for (int i : order_) {
        ...
        const float ox = float(r.x) * WORLD_TO_PX - s.frame_w * 0.5f + s.off_x;
        const float oy = float(r.y) * WORLD_TO_PX - s.frame_h * 0.5f + s.off_y - alt;


        const float pal = float(r.owner) + (r.unpowered ? float(dim_offset_) : 0.0f);
        put_instance(p, ox, oy, float(frame_for(r)), pal);
        if (s.turret_first >= 0) {
            const int tf = s.turret_first + facing_to_frame(r.turret, 32, true);
            put_instance(p, ox, oy, float(tf), pal);
        }
        ...
        for (int k = 0; k < s.rotor_count; ++k) {
            ...
            const ra::WVec fwd = ra::direction_of(r.facing);
            const ra::WVec right = ra::direction_of(ra::wrap_angle(r.facing - 256));
            const float rx = ox + float(int64_t(fwd.x) * rot.ox / 1024 + int64_t(right.x) * rot.oy / 1024) * WORLD_TO_PX;
            const float ry = oy + float(int64_t(fwd.y) * rot.ox / 1024 + int64_t(right.y) * rot.oy / 1024) * WORLD_TO_PX
                           - float(rot.oz) * WORLD_TO_PX;
    ...
    for (const ra::RenderSprite& s : sprites_scratch_) {
        ...
        if (s.weapon >= 0) {
            ...
            int frame = seq.first_frame + s.frame;
            if (es.facings > 1) frame = seq.first_frame + facing_to_frame(s.facing, es.facings, false) * es.length + s.frame;
            put_instance(p, float(s.x) * WORLD_TO_PX - es.w * 0.5f, float(s.y) * WORLD_TO_PX - es.h * 0.5f - alt, float(frame), 0.0f);
        }
    }

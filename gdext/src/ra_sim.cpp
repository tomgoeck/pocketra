...
static void put_custom_data(float*& p, float frame, float player) {
    const int index = int(std::lround(frame));
    *p++ = float(index & 255) / 255.0f;
    *p++ = float((index >> 8) & 255) / 255.0f;
    *p++ = float(int(std::lround(player)) + 1) / 255.0f;
    *p++ = 0.0f;
}
...
static void put_instance(float*& p, float ox, float oy, float frame, float player) {
    ...
    *p++ = 0.0f; *p++ = 1.0f; *p++ = 0.0f; *p++ = oy;
    put_custom_data(p, frame, player);
}
...
void RaSim::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("harvester_info", "owner"), &RaSim::harvester_info);
...
int RaSim::define_type(const Dictionary& def) {
    ...
    t.repulsable = bool(def.get("repulsable", true));
    t.repulsion_speed = int(def.get("repulsion_speed", -1));
    t.ideal_separation = int(def.get("ideal_separation", 1706));
...
void RaSim::enable_bot(int owner, const Dictionary& params) {
    ...
    p.harv_stall_ticks = int(params.get("harv_stall_ticks", p.harv_stall_ticks));
    p.harvesters_extra = int(params.get("harvesters_extra", p.harvesters_extra));
    ...
    p.straggler_min_cells = int(params.get("straggler_min_cells", p.straggler_min_cells));
    p.straggler_interval = int(params.get("straggler_interval", p.straggler_interval));
    p.gather_ready_percent = int(params.get("gather_ready_percent", p.gather_ready_percent));
    p.gather_repicks_max = int(params.get("gather_repicks_max", p.gather_repicks_max));
    p.no_flee_ticks = int(params.get("no_flee_ticks", p.no_flee_ticks));
    p.counter_push_on = int(params.get("counter_push_on", p.counter_push_on));


    p.defense_aa_max = int(params.get("defense_aa_max", p.defense_aa_max));
    p.defense_army_percent = int(params.get("defense_army_percent", p.defense_army_percent));
    p.defense_growth_tick = int(params.get("defense_growth_tick", p.defense_growth_tick));


    p.air_soft_targets = int(params.get("air_soft_targets", p.air_soft_targets));
    p.naval_escort = int(params.get("naval_escort", p.naval_escort));
    p.naval_escort_per_siege = int(params.get("naval_escort_per_siege", p.naval_escort_per_siege));
    p.naval_escort_cells = int(params.get("naval_escort_cells", p.naval_escort_cells));
    p.naval_retreat_cells = int(params.get("naval_retreat_cells", p.naval_retreat_cells));
    p.naval_hunt_cloaked = int(params.get("naval_hunt_cloaked", p.naval_hunt_cloaked));
    p.naval_memory_ticks = int(params.get("naval_memory_ticks", p.naval_memory_ticks));
    p.naval_siege_range_cells = int(params.get("naval_siege_range_cells", p.naval_siege_range_cells));


    {
        const PackedInt32Array qb = params.get("queue_budget", PackedInt32Array());
        for (int i = 0; i < ra::BQ_COUNT && i < qb.size(); ++i) p.queue_budget[i] = qb[i];
    }
    p.naval_unit_limit = int(params.get("naval_unit_limit", p.naval_unit_limit));
    p.naval_escort_per_bombard = int(params.get("naval_escort_per_bombard", p.naval_escort_per_bombard));
    p.continuous_production = int(params.get("continuous_production", p.continuous_production));
    p.wave_value = int(params.get("wave_value", p.wave_value));


    p.opening_plan = int(params.get("opening_plan", p.opening_plan));
    p.opening_refineries = int(params.get("opening_refineries", p.opening_refineries));
    p.opening_factory_after = int(params.get("opening_factory_after", p.opening_factory_after));
    p.opening_production_free = int(params.get("opening_production_free", p.opening_production_free));
    p.opening_defense_per = int(params.get("opening_defense_per", p.opening_defense_per));
    p.opening_attack_ticks = int(params.get("opening_attack_ticks", p.opening_attack_ticks));
...
Dictionary RaSim::bot_stats(int owner) {
    ...
    d["sammler_still_max"] = int(b.stat_harv_worst);
    d["sammler_still_spitze"] = int(b.stat_harv_peak);
    d["sammler_still_proben"] = int(b.stat_harv_samples);
    d["sammler_still_erz"] = int(b.stat_harv_ore);


    d["sammler_soll"] = int(world_.bot_harvester_target(owner));
    Array zust;
    for (int k = 0; k < ra::HARV_STATES; ++k) zust.push_back(int(b.stat_harv_state[k]));
    d["sammler_still_zustand"] = zust;


    d["schwere_verluste"] = int(b.stat_attacks_heavy_loss);
    d["trupp_gather_ticks"] = int(b.stat_squad_gather_ticks);
    d["trupp_marsch_ticks"] = int(b.stat_squad_march_ticks);
    d["trupp_kontakte"] = int(b.stat_squad_contacts);
    d["trupp_bis_schuss"] = int(b.stat_squad_contacts > 0
                                ? b.stat_squad_contact_ticks / b.stat_squad_contacts : -1);
    d["trupp_ohne_schuss"] = int(b.stat_squads_no_contact);
    d["sammel_neu"] = int(b.stat_gather_repicks);
    d["nachzuegler_stops"] = int(b.stat_straggler_stops);


    int flak = 0, spread = 0, uncovered = 0;
    world_.bot_defense_stats(owner, flak, spread, uncovered);
    d["flaktuerme"] = flak;
    d["turm_streuung"] = spread;
    d["ungedeckt"] = uncovered;

    d["luftangriffe"] = int(b.stat_air_attacks);
    d["flieger_verloren"] = int(b.stat_air_lost);
    d["schiffe_verloren"] = int(b.stat_ships_lost);
    d["schiffe_versenkt"] = int(b.stat_ships_killed);


    d["gebaut_fahrzeuge"] = int(b.stat_built_q[ra::BQ_VEHICLE]);
    d["gebaut_infanterie"] = int(b.stat_built_q[ra::BQ_INFANTRY]);
    d["gebaut_luft"] = int(b.stat_built_q[ra::BQ_AIRCRAFT]);
    d["gebaut_schiffe"] = int(b.stat_built_q[ra::BQ_NAVAL]);


    int32_t qv[ra::BQ_COUNT];
    world_.bot_queue_values(owner, qv);
    d["armee_land"] = int(qv[ra::BQ_VEHICLE] + qv[ra::BQ_INFANTRY]);
    d["armee_marine"] = int(qv[ra::BQ_NAVAL]);


    d["t_raffinerie"] = int(b.stat_t_refinery);
    d["t_kaserne"] = int(b.stat_t_barracks);
    d["t_fabrik"] = int(b.stat_t_factory);
    d["t_radar"] = int(b.stat_t_radar);
    return d;
}
...
Array RaSim::harvester_info(int owner) const {
    Array out;
    for (size_t i = 0; i < world_.actor_count(); ++i) {
        const ra::Actor& a = world_.actor(i);
        if (!a.alive || a.owner != owner || !world_.type(a.type).harvester) continue;
        const ra::Harvest& h = world_.harvest(i);
        Dictionary d;
        d["id"] = int(a.id);
        d["x"] = int(world_.mobile(i).cell.x);
        d["y"] = int(world_.mobile(i).cell.y);
        d["zustand"] = int(h.state);
        d["still"] = int(h.stall_ticks);
        d["ballen"] = int(h.bales);
        d["automat"] = int(h.automated ? 1 : 0);
        d["claim"] = int(h.claim);
        d["proc"] = int(h.proc);
